...
<div class="search fn-clear">
    <form id="search_form" class="search_form_reset" method="get" action="<?= Yf_Registry::get('url') ?>">
        <input class="text w150" type="text" name="common_name" value="<?=request_string('common_name')?>" placeholder="<?=__('请输入商品名称')?>"/>
                ...
                <tr id="tr_common_id_<?= $item['common_id']; ?>">
                    <td class="tl th" colspan="99">
                        <label class="checkbox">
							<input class="checkitem" type="checkbox" name="chk[]" value="<?= $item['common_id'] ?>"></label><?=__('平台货号：')?><?= $item['common_id']; ?>
                    </td>
                </tr>
                <tr>
                    <td class="tl">
...
<!--                                <p>--><?//= ($item['common_code'] ? sprintf("商品条码：%s", $item['common_code']) : '') ?><!--</p>-->
                    ...
                    <td>
                        <span class="edit">
							<a  href="javascript:void(0)" common-id='<?=$item['common_id']?>' first_rate='<?=$item['common_cps_rate']?>' second_rate='<?=$item['common_second_cps_rate']?>' third_rate='<?=$item['common_third_cps_rate']?>' id="set_commission"><i class="iconfont icon-setting"></i><?=__('设置')?></a></span>
                        <span class="del"><a data-param="{'id':'<?= $item['common_id'] ?>','ctl':'Seller_Goods','met':'deleteGoodsCommon'}" href="javascript:void(0)"><i class="iconfont icon-lajitong"></i><?=__('删除')?></a></span>
                    </td>
                </tr>
...
<?php } ?>
	...
	<div id="dialog_directseller" class="eject_con" style="display:none;">
		...
		<div class="eject_con mb10">
			<div class="bottom"><a id="btn_directseller_shop_name" class="button bbc_seller_submit_btns" href="javascript:void(0);"><?=__('提交')?></a></div>
		</div>
	</div>
	<script>
		//设置分佣比例
        ...
        $('#btn_directseller_shop_name').on('click', function(){
            ...
            if(common_cps_rate!='') 
			{
                $.post(SITE_URL + '?ctl=Distribution_Seller_Setting&met=editDirectsellerGoods&typ=json',
                    {common_id: common_id, common_cps_rate: common_cps_rate,common_second_cps_rate: common_second_cps_rate,common_third_cps_rate: common_third_cps_rate},
                    function(d) {
                        if(d.status == 200) {
                            var data = d.data;                 
