...
<!--                                      <div id="stock_area_item" data-widget="tab-content" data-area="2" style="display: none;">
                        ...
                        <span class="class_drap" >
                                   ...
                                   <select id="address_location" name="address_location"  class="sgcategory valid" style="margin-top:4px" onchange="location_jumpMenu(<?= $cat_id ?>)">
                                       <option value=""  <?php if( empty( $address_id )) {echo "selected";}?>  > <?=__('收货地址距离排序...')?> </option>
                                       <?php if ( !empty($userAddressList) ) { ?>
                                           <?php foreach ($userAddressList as $key => $val) { ?>
                                               <option  value="<?= $val['user_address_id']; ?>"  <?php if($address_id== $val['user_address_id']){echo "selected";}?>  > <?= $val['user_address_address']; ?></option>
                                           <?php } ?>
                                       <?php } ?>
                               </select>
						</span>
				...
				<ul class="cr_xie clearfix">
					...
					<li>
						...
						<?php if(!$val['common_is_virtual']){?><a  class="cr_xie_lia2" onclick="addCart(<?=(@$val['goods_id'])?>,<?=(@$val['shop_owner'])?>,<?=(@$val['dist_owner'])?>)"><span class="cr_xie_lia2_span1 cr_xie_lia2_span1—3 iconfont icon-wodegouwuche bbc_color"></span><span class="cr_xie_lia2_span2 cr_xie_lia2_span2-1 "><?=__('加入购物车')?></span></a><?php }else{?><a  class="cr_xie_lia2" onclick="nowBuy(<?=(@$val['goods_id'])?>,<?=(@$val['shop_owner'])?>,<?=(@$val['dist_owner'])?>)"><span class="cr_xie_lia2_span1 cr_xie_lia2_span1—3"></span><span class="cr_xie_lia2_span2 cr_xie_lia2_span2-1"><?=__('立即购买')?></span></a><?php }?></div>
					</li>
					<?php 	endforeach;?>
				</ul>
...
<div id="login_content" style="display:none;">
	...
	<script>
		...
		function location_jumpMenu(cartid){
		  var  loc =   $("#address_location").val();
		  var  lurl  =  SITE_URL + '?ctl=Goods_Goods&met=goodslist&cat_id='+cartid
		    if (loc.length  >0 ){
                lurl  =  SITE_URL + '?ctl=Goods_Goods&met=goodslist&cat_id='+cartid + "&act=location&ad_id=" + $("#address_location").val();
            }
            eval("this.location='" + lurl +"'");


		}
		//加入购物车
		window.addCart = function(e,i,j)
		{
			if ($.cookie('key'))
			{
				if(i == 1 || j == 1)
