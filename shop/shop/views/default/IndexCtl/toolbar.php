...
<link rel="stylesheet" type="text/css" href="<?= $this->view->css ?>/coupon.css"/>
        ...
        <div class="toolbar-panels J-panel">
            ...
            <div style="visibility: hidden;" class="J-content toolbar-panel tbar-panel-assets toolbar-animate-out">
                <div class="padd">
                    ...
                    <div class="other_voucher">
                        <div id="J-coupon-items">
                            <div class="coupon-wrap" id="unget_conpons">
                                <div class="coupon-type sku_conpon_title">可领取的券</div>
                                <?php
                                foreach($shop_vouchers_row['items'] as $key=>$value) {
                                    $voucher_title = "仅可用于".$value['shop_name'];
                                    $voucher_time = "有效期".date('Y-m-d',strtotime($value['voucher_t_start_date'])).' - '.date('Y-m-d',strtotime($value['voucher_t_end_date']));
                                    ?>
                                    <div class="coupon-item sku_coupon_item jing" id="sc_<?=$value['voucher_t_id']?>">
                                        <div class="item-wrap">
                                            <div class="coupon-price">
                                                <span class="coupon-val"><?=format_money($value['voucher_t_price'])?></span>
                                                <span class="coupon-name">代金券</span>
                                            </div>
                                            <div class="coupon-info">
                                        <span class="condition" title="<?=$voucher_title?>">
                                            <?=__('满')?><?=format_money($value['voucher_t_limit'])?><?=__('元可使用')?></span>
                                                <p title="<?=$voucher_title?>"><?=$voucher_title?></p>
                                            </div>
                                            <a class="btn-get" op_type="exchangebtn-toolbar" data-param='{"vid":"<?=$value['voucher_t_id']?>"}'><?=__('立即领取')?></a>
                                            <p class="coupon-time"><?=$voucher_time?></p>
                                        </div>
                                    </div>
                                <?php } ?>
                            </div>
                            <div class="coupon-wrap" id="geted_conpons">
                                <div class="coupon-type sku_conpon_title">已领取的券</div>
                                <?php
                                foreach($vouchers_get['items'] as $key=>$value) {
                                    $voucher_title = __('仅可用于').$value['shop_name'];
                                    $voucher_time = __('有效期').date('Y-m-d',strtotime($value['voucher_start_date'])).' - '.date('Y-m-d',strtotime($value['voucher_end_date']));
                                    ?>
                                    <div class="coupon-item sku_coupon_item jing" id="sc_<?=$value['voucher_t_id']?>">
                                        <div class="item-wrap">
                                            <div class="coupon-price">
                                                <span class="coupon-val"><?=format_money($value['voucher_price'])?></span>
                                                <span class="coupon-name"><?=__('代金券')?></span>
                                            </div>
                                            <div class="coupon-info">
                                                <span class="condition" title="<?=$voucher_title?>">
                                                    <?=__('满')?><?=format_money($value['voucher_limit'])?><?=__('可使用')?></span>
                                                    <p title="<?=$voucher_title?>"><?=$voucher_title?></p>
                                            </div>
                                            <div class="coupon-seal"></div>
                                            <a class="btn-get" href="<?= Yf_Registry::get('url') ?>?ctl=Shop&met=index&id=<?=$value['voucher_shop_id']?>"><?=__('立即使用')?></a>
                                            <p class="coupon-time"><?=$voucher_time?></p>
                                        </div>
                                    </div>
                                <?php } ?>
...
<script type="text/javascript">
    ...
    $(function(){
        //显示要兑换的代金券信息
        $("[op_type='exchangebtn-toolbar']").on('click',function(){
            var data_str = $(this).attr('data-param');
            eval( "data_str = "+data_str);
            var vid = data_str.vid;
            $.post(SITE_URL + "?ctl=Voucher&typ=json&met=receiveVoucher",{vid:vid }, function (data)
            {
                var t=data.status;
                if (200 == t)
                {
                    parent.Public.tips({content: data.msg});
                    $("#sc_" + vid).find(".coupon-info").after('<div class="coupon-seal"></div>');
                    $("#geted_conpons > .sku_conpon_title").after($("#sc_" + vid));
                    $("#unget_conpons > #sc_" + vid).html('');
                }
                else
                {
