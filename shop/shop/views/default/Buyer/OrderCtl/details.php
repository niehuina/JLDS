        ...
        <div class="ncm-order-info">
            <div class="ncm-order-details">
                ...
                <div class="content">
                    ...
                    <dl class="line line2">
                        <dt><?= __('订单编号：') ?></dt>
                        <dd><?= ($data['order_id']) ?><a class="">更多<i class="iconfont icon-iconjiantouxia"></i>
                        ...
                        <dt><?= __('商　　家：') ?></dt>
                        <dd><?= ($data['shop_names']) ?><a class=""><?= __('更多') ?><i
                ...
                <div class="ncm-order-condition">
                    ...
                    <ul>
                        <?php if ($data['payment_id'] != PaymentChannlModel::PAY_CONFIRM): ?>
                            <li><?= __('1. 您尚未对该订单进行支付，请') ?><?php if ($data['payment_id'] != PaymentChannlModel::PAY_CONFIRM && $data['order_sub_pay'] == Order_StateModel::SUB_SELF_PAY) { ?>
                                    <a
                                    href="<?= Yf_Registry::get('paycenter_api_url') ?>?ctl=Info&met=pay&uorder=<?= ($data['payment_number']) ?>"
                                    class="ncbtn-mini ncbtn-bittersweet bbc_btns"><i></i><?= __('支付订单') ?>
                                    </a><?php } else { ?><?= __('联系主管账号支付订单') ?><?php } ?><?= __('以确保商家及时发货。') ?></li>
                            <li><?= __('2. 如果您未对该笔订单进行支付操作，系统将于') ?>
                                <time><?= ($data['cancel_time']) ?></time><?= __('自动关闭该订单') ?>。
                            </li>

                            <?php if ($data['buyer_user_id'] == Perm::$userId && $data['order_sub_pay'] == Order_StateModel::SUB_SELF_PAY) { ?>
                                <li><?= __('3. 如果您不想购买此订单的商品，请选择 ') ?><a
                                            onclick="cancelOrder('<?= $data['order_id'] ?>')"
                                            class="ncbtn-mini bbc_btns"><?= __('取消订单') ?></a><?= __('操作。') ?></li>
                            <?php } else { ?>
                                <li><?= __('3. 如果您不想购买此订单的商品，需要采购子账号进行取消订单操作。') ?></li>
                            <?php } ?>

                        <?php else: ?>
                            <li><?= __('1. 如果您不想购买此订单的商品，请选择 ') ?><a onclick="cancelOrder('<?= $data['order_id'] ?>')"
                                                                     class="ncbtn-mini bbc_btns"><?= __('取消订单') ?></a><?= __('操作。') ?>
                            </li>
                            <li><?= __('2. 如果您未对该笔订单进行支付操作，系统将于') ?>
                                <time><?= ($data['cancel_time']) ?></time><?= __('自动关闭该订单') ?>。
                            </li>
                        ...
                        <?php if ($data['buyer_user_id'] == Perm::$userId) { ?>
                            <li><?= __('3. 如果您想取消购买，请与商家沟通后对订单进行') ?>
                                <?php if (strstr($data['payment_name'], '白条支付')) { ?>
                                    <a onclick="javascript:alert('白条支付的订单，请联系商家线下退款');"
                                       class="ncbtn-mini bbc_btns"><?= __('申请退款') ?></a>
                                <?php } else { ?>
            ...
            <?php if ($data['order_status'] == Order_StateModel::ORDER_CANCEL): ?>
                <div class="ncm-order-condition">
                    ...
                    <ul>
                        <li><?= ($data['cancel_identity']) ?><?= __('于') ?>
                            <time><?= ($data['order_cancel_date']) ?></time><?= __('取消了订单（') ?><?= ($data['order_cancel_reason']) ?><?= __('）') ?>
                        </li>
                    </ul>
                </div>
            <?php endif; ?>
        ...
        <table>
            ...
            <tr>
                            ...
                            <tr class="tr_con">
                                <td class="order_goods">
                                    ...
                                    <?php if (isset($ogval['order_spec_info']) && $ogval['order_spec_info']) { ?>
                                        <dd style="float: left;margin-left: 17px"><strong><?= __('规格') ?>：</strong>&nbsp;&nbsp;<em><?= $ogval['order_spec_info'][0] . ',' . $ogval['order_spec_info'][1]; ?></em>
                                        </dd>
                                    <?php } ?>
                                ...
                                <td class="td_color widt4">
                                    ...
                                    if ($data['payment_id'] == PaymentChannlModel::PAY_CONFIRM) {
                                        ...
                                        if (($data['order_status'] == Order_StateModel::ORDER_RECEIVED || $data['order_status'] == Order_StateModel::ORDER_FINISH) && $data['order_refund_status'] == Order_StateModel::ORDER_REFUND_NO) {
                                            ...
                                            <?php } else { ?>
                                                <p><a target="_blank"
                                                      href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Return&met=index&act=add&oid=<?= ($data['order_id']) ?>&gid=<?= ($ogval['order_goods_id']) ?>"
                ...
                <td class="td_rborder widt5">
				     <span class="fls">
				        <em class="type-name"><?= __('总额：') ?></em><strong><?= format_money($data['order_goods_amount']) ?></strong>
                         <!--<br/>--><? /*=($data['payment_name'])*/ ?>
				     </span>
                    <br/>
                    <span class="fls">
                    ...
                    <?php if ($data['order_shop_benefit']) { ?><span
                            class="td_sale bbc_btns"><?= ($data['order_shop_benefit']) ?></span><?php } ?>
                </td>
                <!--E 订单金额 -->
                ...
                <td class="td_rborder">
                    ...
                    <?php if ($data['order_status'] == Order_StateModel::ORDER_WAIT_CONFIRM_GOODS) { ?>
                        <a style="position:relative;"
                           onmouseover="show_logistic('<?= ($data['order_id']) ?>','<?= ($data['order_shipping_express_id']) ?>','<?= ($data['order_shipping_code']) ?>')"
                ...
                <td class="td_rborder td_rborder_reset">
                    ...
                    <?php if ($data['order_status'] == Order_StateModel::ORDER_WAIT_PAY): ?>
                        ...
                        <?php if ($data['payment_id'] != PaymentChannlModel::PAY_CONFIRM && $data['order_sub_pay'] == Order_StateModel::SUB_SELF_PAY): ?>
                            <p>
                                <a target="_blank"
                                   onclick="payOrder('<?= $data['payment_number'] ?>','<?= $data['order_id'] ?>')"
                                   class="to_views "><i
                                            class="iconfont icon-icoaccountbalance pay-botton"></i><?= __('订单支付') ?></a>
                            </p>
                        <?php endif; ?>

                        <?php if ($data['buyer_user_id'] == Perm::$userId) { ?>
                            <p><a onclick="cancelOrder('<?= $data['order_id'] ?>')" class="to_views"><i
                    ...
                    <?php if ($data['buyer_user_id'] == Perm::$userId) { ?>
                        ...
                        <?php if ($data['order_status'] == Order_StateModel::ORDER_WAIT_CONFIRM_GOODS && $data['can_confirm_order']): ?>
                            ...
                            <p><a onclick="confirmOrder('<?= $data['order_id'] ?>')" class="to_views "><i
                                            class="iconfont icon-duigou1"></i><?= __('确认收货') ?></a></p>
                        <?php endif; ?>

                        <?php if ($data['order_status'] == Order_StateModel::ORDER_FINISH): ?>
                            <?php if (!$data['order_buyer_evaluation_status']): ?>
                                <p><a target="_blank"
                                      href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=evaluation&act=add&order_id=<?= ($data['order_id']) ?>"
                                      class="to_views"><i
                                                class="iconfont icon-woyaopingjia icon_size22"></i><?= __('我要评价') ?></a>
                                </p>
                            <?php endif; ?>
                            <?php if ($data['order_buyer_evaluation_status']): ?>
                                <p><a target="_blank"
                                      href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=evaluation&act=add&order_id=<?= ($data['order_id']) ?>"
                                      class="to_views"><i
                                            class="iconfont icon-woyaopingjia icon_size22"></i><?= __('追加评价') ?></a>
                            <?php endif; ?></p>
                        <?php endif; ?>
                    <?php } ?>
                    ...
                    <p>
                        <a onclick="againOrder('<?= $data['order_id'] ?>')">
                            <i class="iconfont icon-zaiqigoumai icon_size22"></i><?= __('再来一单') ?>
                        </a>
                    </p>
                    ...
                    <?php if (($data['order_status'] == Order_StateModel::ORDER_CANCEL || $data['order_status'] == Order_StateModel::ORDER_FINISH) && (!$data['order_buyer_hidden'])): ?>
                        <p>
                            <a onclick="hideOrder('<?= $data['order_id'] ?>')"><i
                    ...
                    <?php if (($data['order_status'] == Order_StateModel::ORDER_CANCEL || $data['order_status'] == Order_StateModel::ORDER_FINISH) && $data['order_buyer_hidden'] == Order_BaseModel::IS_BUYER_HIDDEN) : ?>
                        ...
                        <p>
                            <a onclick="delOrder('<?= $data['order_id'] ?>')" class="to_views">
                                <i class="iconfont icon-lajitong icon_size22"></i><?= __('彻底删除') ?>
                            </a>
                        </p>
