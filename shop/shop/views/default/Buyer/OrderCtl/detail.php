          ...
          <table>
              ...
              <tbody class="tboy">
                                ...
                                <?php if($data['buyer_user_id'] == Perm::$userId){ ?>
                                    ...
                                    <?php if($ogval['goods_return_status'] == Order_StateModel::ORDER_GOODS_RETURN_NO){?>
                                            <?php if($ogval['goods_returns_setting'] != Goods_ReturnsModel::GOOD_NOT_RETURN){?>
                                                ...
                                                <?php }else{ ?>
                                                <p><a target="_blank" href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Return&met=index&act=add&oid=<?=($ogval['order_id'])?>&gid=<?=($ogval['order_goods_id'])?>" class="to_views"><?=__('退款/退货')?></a></p>
                                                <?php } ?>
                                            <?php } ?>
                                           <?php } ?>
                                    <?php }?>
				...
				<td class="td_rborder td_rborder_reset">
                    ...
                    <p><a onclick="againOrder('<?=$data['order_id']?>')"><i class="iconfont icon-zaiqigoumai icon_size22"></i><?=__('再来一单')?></a></p>
