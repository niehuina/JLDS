          ...
          <table>
              ...
              <!-- 下单时间，订单号，店铺名称    -->
				...
				<td class="td_rborder td_rborder_reset">
                    ...
                    <p><a onclick="againOrder('<?=$val['order_id']?>')"><i class="iconfont icon-zaiqigoumai icon_size22"></i><?=__('再来一单')?></a></p>

                    <?php if(($val['order_status'] == Order_StateModel::ORDER_CANCEL || $val['order_status'] == Order_StateModel::ORDER_FINISH) && $recycle != 1):?>
                      <p>
                        <a onclick="hideOrder('<?=$val['order_id']?>')"><i class="iconfont icon-lajitong icon_size22"></i><?=__('删除订单')?></a></p>
