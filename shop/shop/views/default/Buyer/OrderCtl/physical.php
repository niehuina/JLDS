            ...
            <?php if ($data['items']) { ?>
                <?php foreach ($data['items'] as $key => $val): ?>
                    ...
                    <tr class="tr_title">
                        <th colspan="8" class="order_mess clearfix">
                            <p class="order_mess_one">
                                ...
                                <a target="_blank"
                                   href="<?= Yf_Registry::get('url') ?>?ctl=Shop&met=index&id=<?= ($val['shop_id']) ?>"><i
                                            class="iconfont icon-icoshop"></i><?= ($val['shop_names']) ?></a>
                    ...
                    <tr>
                                    ...
                                    <tr class="tr_con">
                                        <td class="order_goods">
                                            ...
                                            <?php if (isset($ogval['order_spec_info']) && $ogval['order_spec_info']) { ?>
                                                <dd style="float: left;margin-left: 17px"><strong><?= __('规格') ?>
                                                        ：</strong>&nbsp;&nbsp;<em><?= $ogval['order_spec_info'][0] . ',' . $ogval['order_spec_info'][1]; ?></em>
                                                </dd>
                                            <?php } ?>

                                            <?php if ($ogval['order_goods_benefit']) { ?><em
                                                    class="td_sale bbc_btns small_details"><?= ($ogval['order_goods_benefit']) ?></em><?php } ?>
                                        </td>
                                        <td class="td_color widt1"><?= format_money($ogval['goods_price']) ?></td>
                                        <td class="td_color widt2">
                                            <i class="iconfont icon-cuowu" style="position:relative;font-size: 12px;"></i> <?= ($ogval['order_goods_num']) ?>
                                        </td>
                                        <td class="td_color widt4">
                                            ...
                                            if ($val['payment_id'] == PaymentChannlModel::PAY_CONFIRM) { ?>
                                                ...
                                                if (($val['order_status'] == Order_StateModel::ORDER_RECEIVED || $val['order_status'] == Order_StateModel::ORDER_FINISH)
                                                    && $val['order_refund_status'] == Order_StateModel::ORDER_REFUND_NO) {
                                                    ?>
                                                    <?php
                                            ...
                                            <?php } else {
                                                ...
                                                if ((($val['order_status'] == Order_StateModel::ORDER_PAYED && $ogval['goods_return_status'] == Order_StateModel::ORDER_GOODS_RETURN_NO)
                                                        || ($val['order_status'] == Order_StateModel::ORDER_WAIT_CONFIRM_GOODS && $ogval['goods_refund_status'] == Order_StateModel::ORDER_GOODS_RETURN_NO))
                                                    && !$val['order_source_id'] && $val['order_refund_status'] == Order_StateModel::ORDER_REFUND_NO
                                                    && $ogval['order_goods_num'] > $ogval['order_goods_returnnum'] && $ogval['goods_price'] > 0
                                                ) {
                                                    ?>
                                                    <?php
                                                    //白条支付的订单需要线下进行退款/退货操作
                                            ...
                                            <p>
                                                <?php if (($val['order_status'] == Order_StateModel::ORDER_FINISH && $val['complain_status'])
                                                    || ($val['order_status'] != Order_StateModel::ORDER_CANCEL && $val['order_status'] != Order_StateModel::ORDER_WAIT_PAY)) { ?>
                                                    <a target="_blank"
				     ...
				     <span class="fls"><em
                                 class="type-name"><?= __('订单总额：') ?></em><strong><?= format_money($val['order_goods_amount']) ?></strong>
                         <!--<br/>--><? /*=($val['payment_name'])*/ ?>
             </span>

                            <br/>
                            <span class="fls"><em
                                        class="type-name"><?= __('运费：') ?></em><strong><?php if ($val['order_shipping_fee'] > 0): ?><?= format_money($val['order_shipping_fee']) ?><?php else: ?><?= __('免运费') ?><?php endif; ?></strong>
                    </span>

                            <br/>
                            <span class="fls"><em
                                        class="type-name"><?= __('应付：') ?></em><strong><?= format_money($val['order_payment_amount']) ?></strong>
             </span>

                            <?php if ($val['order_shop_benefit']) { ?><span
                                    class="td_sale bbc_btns"><?= ($val['order_shop_benefit']) ?></span><?php } ?>
                        </td>
                        <!--E 订单金额 -->
                        ...
                        <td class="td_rborder">
                            ...
                            <?php if ($val['order_status'] == Order_StateModel::ORDER_WAIT_CONFIRM_GOODS) { ?>
                                <a style="position:relative;"
                                   onmouseover="show_logistic('<?= ($val['order_id']) ?>','<?= ($val['order_shipping_express_id']) ?>','<?= ($val['order_shipping_code']) ?>')"
                                   onmouseout="hide_logistic('<?= ($val['order_id']) ?>')">
                                    <i class="iconfont icon-icowaitproduct rel_top2"></i><?= __('物流信息') ?>
                                    <div style="display: none;" id="info_<?= ($val['order_id']) ?>"
                                         class="prompt-01"></div>
                                </a>
                            <?php } ?>
                        ...
                        <td class="td_rborder td_rborder_reset">
                            <?php if (!$val['order_source_id']) { ?> <!--分销商不能操作SP订单  -->
                                ...
                                <p><a onclick="againOrder('<?= $val['order_id'] ?>')"><i
                                                class="iconfont icon-zaiqigoumai icon_size22"></i><?= __('再来一单') ?></a>
                                </p>

                                <?php if (($val['order_status'] == Order_StateModel::ORDER_CANCEL || $val['order_status'] == Order_StateModel::ORDER_FINISH) && $recycle != 1): ?>
                                    <p><a onclick="hideOrder('<?= $val['order_id'] ?>')"><i
                                                    class="iconfont icon-lajitong icon_size22"></i><?= __('删除订单') ?></a>
                                    </p>
                                ...
                                <?php if ($val['order_status'] == Order_StateModel::ORDER_WAIT_PAY): ?>
                                    ...
                                    <?php if ($val['payment_id'] != PaymentChannlModel::PAY_CONFIRM && $val['order_sub_pay'] == Order_StateModel::SUB_SELF_PAY) { ?>
                                        <p>
                                            <a target="_blank"
                                               onclick="payOrder('<?= $val['payment_number'] ?>','<?= $val['order_id'] ?>')"
                                               class="to_views "><i
                                                        class="iconfont icon-icoaccountbalance pay-botton"></i><?= __('订单支付') ?>
                                            </a>
                                        </p>
                                    <?php } ?>

                                    <p><a onclick="cancelOrder('<?= $val['order_id'] ?>')" class="to_views"><i
                                                    class="iconfont icon-quxiaodingdan"></i><?= __('取消订单') ?></a></p>
                                ...
                                <?php if ($val['order_status'] == Order_StateModel::ORDER_FINISH): ?>
                                    <?php if (!$val['order_buyer_evaluation_status']): ?>
                                        <p><a target="_blank"
                                              href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=evaluation&act=add&order_id=<?= ($val['order_id']) ?>"
                                              class="to_views"><i
                                                        class="iconfont icon-woyaopingjia icon_size22"></i><?= __('我要评价') ?>
                                            </a></p>
                                    <?php endif; ?>
                                    <?php if ($val['order_buyer_evaluation_status'] == 1): ?>
                                        <p><a target="_blank"
                                              href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=evaluation"
                                              class="to_views"><i class="iconfont icon-woyaopingjia icon_size22"></i>
                                ...
                                <?php if ($recycle): ?>
                                    ...
                                    <p><a onclick="restoreOrder('<?= $val['order_id'] ?>')"><i
                                                    class="iconfont icon-huanyipi"></i><?= __('还原订单') ?></a></p>

                                    <p><a onclick="delOrder('<?= $val['order_id'] ?>')" class="to_views"><i
                                                    class="iconfont icon-lajitong icon_size22"></i><?= __('彻底删除') ?></a>
                                    </p>
