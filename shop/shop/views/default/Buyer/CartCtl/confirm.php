	...
	<div class="wrap">
        ...
        <ul class="receipt_address clearfix" id="shop_address" style="display: none">
            <div class="address_list">
                <?php if(isset($shopEntityList)){$total = 0; $total_dian_rate = 0; foreach ($shopEntityList as $key => $value) {
                    ?>
                        <li id="addr<?=($value['entity_id'])?>">
                        <input type ="hidden" id="address_id" value="<?=($value['entity_id'])?>">
                        <input type="hidden" id="shop_address_province_id" value="<?=($value['shop_address_province_id'])?>">
                        <input type="hidden" id="shop_address_city_id" value="<?=($value['shop_address_city_id'])?>">
                        <input type="hidden" id="shop_address_area_id" value="<?=($value['shop_address_area_id'])?>">

                        <h5><?=($value['entity_name'])?></h5>
                        <p class="addr-len"><?=($value['province'])?> <?=($value['city'])?><?=($value['district'])?><?=($value['street'])?><?=($value['entity_xxaddr'])?></p><span class="phone"><?=($value['entity_tel'])?></span>
			...
			<ul class="cart_goods_list clearfix">
				<li>
						...
						<?php if($increaval['goods_base']['increase_info']){ ?>
							<div class="more-buy">
								...
								<div class="inline morder-buy-con">
									<p class="sel-goods"><span><?=__('购物满')?><?=(Web_ConfigModel::value('monetary_unit'))?><?php echo $increaval['goods_base']['increase_info']['rule'][0]['rule_price'];?><?php if($increaval['goods_base']['increase_info']['rule'][0]['rule_goods_limit'] > 0){?>，<?=__('最多可购')?><?php echo $increaval['goods_base']['increase_info']['rule'][0]['rule_goods_limit'];?><?=__('件')?><?php }?></span><i class="icon"></i></p>
									<!-- 点击.sel-goods下拉列表 -->
														...
														<?php foreach($increaseval['redemption_goods'] as $redempotionkey=>$redempotionval){
															...
															<tr><td>
																	<div class="clearfix bgf item-li <?php echo $increaval['goods_base']['increase_info']['shop_id'];?>">
																		...
																		<p class="tit-tip">
																			<input class="select_increase" rule_id="<?php echo $increaseval['rule_id'];?>" goods_price="<?php echo $redempotionval['redemp_price'];?>" shop_price="<?=(number_format($val['sprice'],2,'.',''))?>" shop_id="<?php echo $increaval['goods_base']['increase_info']['shop_id'];?>" type="checkbox">
																		...
																		<ul class="nctouch-cart-item">
																			...
																			<li class="buy-item">
																				<div class="bgf6 buy-li pd10">
																					<div class="goods-pic">
																						<a target="_blank" href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&gid=<?=($redempotionval['goods_id'])?>">
																				...
																				<div class="jia-shop clearfix">
																					<p class="fl"><?=__('加价购')?><em><?=(Web_ConfigModel::value('monetary_unit'))?><?= $redempotionval['redemp_price'];?></em></p>
																					<div class="fr mrt4 JS_operation">
																						<div class="num-sel">
																							<a class="declick" href="javascript:;">-</a>
																							<input class="increase_num" goods_id="<?php echo $redempotionval['goods_id'];?>" data-max="<?php echo $increaseval['rule_goods_limit'];?>" type="text" value="1">
																							<a class="inclick">+</a>
																						</div>
																					</div>
																				</div>
					...
					<div class="goods_remark clearfix">
						<p class="remarks" style="width: 100%; text-align: left"><span><?=__('备注：')?></span><input type="text" class="remarks_content" name="remarks" id="<?=($key)?>" placeholder="<?=__('限45个字（定制类商品，请将购买需求在备注中做详细说明）')?>"><?=__('提示：请勿填写有关支付、收货、发票方面的信息')?></p>
                    <?php if($data['isDispalyShipping']==true){ ?>
                            ...
                            <p class="remarks"><span><?=__('配送方式：')?></span>
                             ...
                             if(count($val['method_list'])>0)
                             {
                                ...
                                <label style="margin-left: 20px;">
                                    <input type="radio"  name="shipping_method" style="width: 15px;height: 15px;" value="<?php print($method['shipping_method_id']);?>"
                                           id="check_<?php print($method['shipping_method_id']); ?>" shop_id="<?=($method['shop_id'])?>"><?php print($method['shipping_method_Name']);?>
                                </label>
                            <?php }}?></p>
						...
						<div class="order_total">
							<p class="clearfix">
								<span><?=__('商品金额')?></span>
								<i id="total_price" class="price<?=($key)?>" total_shop_price="<?=(number_format($val['sprice'],2,'.',''))?>"><?=(number_format($val['sprice'],2,'.',''))?></i>
							</p>
							...
							<?php if(!Web_ConfigModel::value('rate_service_status') ||(Web_ConfigModel::value('rate_service_status') && $val['shop_self_support'] == 'true')){?>
								<?php if($dian_rate > 0 ){ ?>
									<p class="clearfix">
										<span><?=__('会员折扣')?></span>
										<em></em>
										<i><?=__("-")?><i class="shoprate<?=($key)?> shoprate" shop_rate="<?=number_format($dian_rate,2,'.','')?>"><?=number_format($dian_rate,2,'.','')?></i></i>
									</p>
								<?php } ?>
						...
						<?php if($val['voucher_base'] || ($val['shop_self_support'] == 'true' && $data['rpt_list'])){?>
							<p class="inline">
								<select class="select" id="voucher<?=($val['shop_id'])?>">
									<option value="0" shop_id="<?=($val['shop_id'])?>">----------------------------请选择优惠劵----------------------------</option>
...
<!--											--><?php //if($shop_all_cost >= $redval['redpacket_t_orderlimit']){?>
<!--												<option value="--><?//=$redval['redpacket_price']?><!--" red_id="--><?//=($redval['redpacket_id'])?><!--" shop_id="--><?//=($val['shop_id'])?><!--">--><?//=(Web_ConfigModel::value('monetary_unit'))?><!----><?//=($redval['redpacket_price'])?><!--&nbsp;--><?//=($redval['redpacket_title'])?><!--&nbsp;<time>--><?//=($redval['redpacket_start_date'])?><!--</time> --><?//=__('-')?><!-- <time>--><?//=($redval['redpacket_end_date'])?><!--</time></option>-->
<!--											--><?php //}?>
<!--										--><?php //}?>
<!--									--><?php //}?>
					...
					<div class="frank clearfix">
						...
						<?php if(!Web_ConfigModel::value('rate_service_status') ||(Web_ConfigModel::value('rate_service_status') && $val['shop_self_support'] == 'true')){}else{$user_rate = 100;}?>
						<p class="submit" style="text-align: center;" rate="<?php echo $user_rate;?>">
					<span>
						<?=__('订单金额：')?>
						<strong>
							<?=(Web_ConfigModel::value('monetary_unit'))?><i id="total_order" class="total" total_price="<?=(number_format($total,2,'.','') )?>"><?=(number_format($total,2,'.','') )?></i>
                            </strong>
					</span>
							<?php if(!Web_ConfigModel::value('rate_service_status') ||(Web_ConfigModel::value('rate_service_status') && $val['shop_self_support'] == 'true')){?>
								<?php if($user_rate > 0 ){?>
								...
								<?=__('会员折扣：')?>
											<strong>
									-<?=(Web_ConfigModel::value('monetary_unit'))?><i id="total_rate" class="rate_total" rate_total="<?=number_format($total_dian_rate,2,'.','')?>"><?=number_format($total_dian_rate,2,'.','')?></i>
								</strong>
							</span>
									<?php }?>
						...
						<?php $after_total = number_format($total-$total_dian_rate-$promotion_money,2,'.','');?>
							...
							<?=(Web_ConfigModel::value('monetary_unit'))?><i id="total_pay" class="after_total bbc_color" after_total="<?=(number_format($after_total,2,'.','') )?>"><?=(number_format($after_total,2,'.','') )?></i>
						</strong>
					</span>
							<!--					--><?php //echo $user_rate; ?>
