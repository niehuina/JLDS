...
<?php if (!self::$is_partner) { ?>
    <div class="basic-info">
        <dl>
            <dt>
                <p><img width="180" height="80"
                        src="<?php if (!empty($shop_base['shop_logo'])) { ?><?= $shop_base['shop_logo'] ?>!180x50.jpg<?php } else { ?> <?= $this->web['shop_head_logo'] ?>!180x50.jpg <?php } ?>"/>
                </p>
                <a href="index.php?ctl=Seller_Shop_Setshop&met=index&typ=e&"><i
                            class="iconfont icon-shangjiaruzhushenqing rel_top-2"></i><?= __('编辑店铺设置') ?></a>
            </dt>
            <dd>
        ...
        <div class="detail-rate">
            ...
            <ul>
                <li>
                <span>
                    <?php if ($shop_detail['com_desc_scores'] > 0) { ?>
                        <i class="iconfont  icon-jiantouxiangshang bbc_seller_color"></i><?= __('高于') ?>
                    <?php } elseif ($shop_detail['com_desc_scores'] < 0) { ?>
                        <i class="iconfont  icon-jiantouxiangxia "></i><?= __('低于') ?>
                    <?php } else { ?>
                        <i class="iconfont icon-jiantouxiangshang "></i><?= __('等于') ?>
                    <?php } ?>
                </span>
                    <?= __('描述相符：') ?>
                    <em><?= number_format($shop_detail['shop_desc_scores'], 2, '.', '') ?><?= __('分') ?></em>
                </li>
                <li>
                <span>
                    <?php if ($shop_detail['com_service_scores'] > 0) { ?>
                        <i class="iconfont  icon-jiantouxiangshang bbc_seller_color"></i><?= __('高于') ?>
                    <?php } elseif ($shop_detail['com_service_scores'] < 0) { ?>
                        <i class="iconfont  icon-jiantouxiangxia "></i><?= __('低于') ?>
                    <?php } else { ?>
                        <i class="iconfont icon-jiantouxiangshang "></i><?= __('等于') ?>
                    <?php } ?>
                    ...
                    <?= __('服务态度：') ?>
                    <em><?= number_format($shop_detail['shop_service_scores'], 2, '.', '') ?><?= __('分') ?></em>
                </li>
                <li>
                <span><?php if ($shop_detail['com_send_scores'] > 0) { ?>
                        <i class="iconfont  icon-jiantouxiangshang bbc_seller_color"></i><?= __('高于') ?>
                    <?php } elseif ($shop_detail['com_send_scores'] < 0) { ?>
                        <i class="iconfont  icon-jiantouxiangxia "></i><?= __('低于') ?>
                    <?php } else { ?>
                        <i class="iconfont icon-jiantouxiangshang "></i><?= __('等于') ?>
                    <?php } ?></span><?= __('发货速度：') ?>
                    <em><?= number_format($shop_detail['shop_send_scores'], 2, '.', '') ?><?= __('分') ?></em>
                </li>
            </ul>
        </div>
    </div>
        ...
        <div class="m white-panel">
            <div class="pannel_div">
                ...
                <div class="mc">
                    ...
                    <ul class="fn-clear">
                        <li><a class="<?= $goods_state_normal_num ? 'num bbc_border bbc_color' : '' ?>"
                               href="./index.php?ctl=Seller_Goods&met=online&typ=e"><?= __('出售中商品') ?><em
                                        class="bbc_seller_bg"><?= $goods_state_normal_num ? $goods_state_normal_num : '' ?></em></a>
                        </li>
                        <li><a class="<?= $goods_verify_waiting_num ? 'num num1 bbc_border bbc_color' : '' ?>"
                               href="./index.php?ctl=Seller_Goods&met=offline&met=verify&typ=e&op=3"><?= __('待审核商品') ?>
                                <em
                                        class="bbc_seller_bg"> <?= $goods_verify_waiting_num ? $goods_verify_waiting_num : '' ?></em></a>
                        </li>
                        <li><a class="<?= $goods_state_offline_num ? 'num bbc_border bbc_color' : '' ?>"
                               href="./index.php?ctl=Seller_Goods&met=offline&typ=e&op=1"><?= __('仓库中商品') ?> <em
                                        class="bbc_seller_bg"><?= $goods_state_offline_num ? $goods_state_offline_num : '' ?></em></a>
                        </li>
                        <li><a class="<?= $goods_state_illegal_num ? 'num num1  bbc_border bbc_color' : '' ?>"
                               href="./index.php?ctl=Seller_Goods&met=lockup&typ=e&op=2"><?= __('违规下架商品') ?> <em
                                        class="bbc_seller_bg"><?= $goods_state_illegal_num ? $goods_state_illegal_num : '' ?></em></a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <div class="m white-panel">
        <div class="pannel_div">
            ...
            <div class="mc">
                <div class="focus">
                    <span><?= __('近期售出') ?>：
                        <a href="./index.php?ctl=Seller_Trade_Order&met=physical&typ=e&"><?= __('交易中的订单') ?></a></span>
                    <span><?= __('维权提示') ?>：
                        <a href="./index.php?ctl=Seller_Service_Complain&met=index&typ=e&"><?= __('收到维权投诉') ?>&nbsp;<?= $complain_unsolved_count ? $complain_unsolved_count : 0 ?> / <?= $complain_all_count ? $complain_all_count : 0 ?></a></span>
                </div>
                <ul id="order_num_list">
                    <li><a href="./index.php?ctl=Seller_Trade_Order&met=getPhysicalNew&typ=e"><?= __('待付款订单') ?></a>
                    </li>
                    <li><a href="./index.php?ctl=Seller_Trade_Deliver&met=deliver&typ=e"><?= __('待发货订单') ?></a></li>
                    <li><a href="./index.php?ctl=Seller_Service_Return&met=orderReturn&typ=e"
                           class=""><?= __('退款订单') ?></a></li>
                    <li><a href="./index.php?ctl=Seller_Service_Return&met=goodsReturn&typ=e"
                           class=""><?= __('退货订单') ?></a></li>
                </ul>
            </div>
        </div>
                        ...
                        foreach ($shop_top_rows as $key => $shop_top_row) {
                            ...
                            <tr>
                                ...
                                <td class="tl" width="40">
                                    <a target="_blank"
                                       href="<?= Yf_Registry::get('index_page') ?>?ctl=Goods_Goods&met=goods&type=goods&gid=<?= $shop_top_row['goods_id'] ?>">
                                        <img width="32" src="<?= image_thumb($shop_top_row['goods_image'], 60, 60) ?>"/>
                                    </a>
                                </td>
                                <td class="tl">
                                    <a target="_blank"
                ...
                <div class="mc">
                    <div class="content">
                        <?php if ($data['promotion_items']['groupbuy_allow_flag']) { ?>
                            <dl class="tghd">
                                <dt class="p-name"><a
                                            href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Promotion_GroupBuy&met=index&typ=e"><?= __('团购') ?></a>
                                </dt>
                                <dd class="p-ico"></dd>
                                <dd class="p-hint">
                        ...
                        <?php if ($data['promotion_items']['promotion_allow_flag']) { ?>
                            <dl class="zhxs">
                                <dt class="p-name"><a
                                            href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Promotion_Increase&met=index&typ=e&op=list"><?= __('加价购') ?></a>
                                </dt>
                                <dd class="p-ico"></dd>
                                <dd class="p-hint">
                            ...
                            <dl class="xszk">
                                <dt class="p-name"><a
                                            href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Promotion_Discount&met=index&typ=e"><?= __('限时折扣') ?></a>
                                </dt>
                                <dd class="p-ico"></dd>
                            ...
                            <dl class="mjs">
                                <dt class="p-name"><a
                                            href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Promotion_MeetConditionGift&met=index&typ=e"><?= __('满即送') ?></a>
                                </dt>
                                <dd class="p-ico"></dd>
                                <dd class="p-hint"><span>
                        ...
                        <?php if ($data['promotion_items']['voucher_allow_flag']) { ?>
                            <dl class="djq">
                                <dt class="p-name"><a
                                            href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Promotion_Voucher&met=index&typ=e"><?= __('代金券') ?></a>
                                </dt>
                                <dd class="p-ico"></dd>
                                <dd class="p-hint"><span>
