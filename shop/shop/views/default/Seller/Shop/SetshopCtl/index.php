...
<form method="post" id="form">
    ...
    <div class="form-style">
        ...
        <dl>
            <dt><?= __('店铺logo：') ?></dt>
            <dd>
                <p class="pic" style="width:180px;height:80px;"><img id="logo_img"
                                                                     src="<?php if (!empty($re['shop_logo'])) {
                                                                         echo $re['shop_logo'];
                                                                     } ?>" height="80" width="180"/></p>
                <p class="upload-button"><input type="hidden" id="logo" name="shop[shop_logo]"
                                                value="<?= $re['shop_logo'] ?>"/>
                <div id='logo_upload' class="lblock upload-bg"><i
                            class="iconfont icon-tupianshangchuan"></i><?= __('图片上传') ?></div>
                </p>
                <p class="hint"><?= __('此处为店铺页logo；') ?><br/><?= __('建议使用宽200像素*高60像素内的GIF或PNG透明图片；点击下方"确认提交"按钮后生效。') ?>
                </p>
            </dd>
        </dl>
        <dl>
            <dt><?= __('APP logo：') ?></dt>
            <dd>
                <p class="pic" style="width:180px;height:80px;"><img id="app_logo_img"
                                                                     src="<?php if (!empty($re['app_logo'])) {
                                                                         echo $re['app_logo'];
                                                                     } ?>" height="80" width="180"/></p>
                <p class="upload-button"><input type="hidden" id="app_logo" name="shop[app_logo]"
                                                value="<?= $re['app_logo'] ?>"/>
                <div id='app_logo_upload' class="lblock upload-bg"><i
                            class="iconfont icon-tupianshangchuan"></i><?= __('图片上传') ?></div>
                </p>
                <p class="hint"><?= __('此处为APP logo；') ?>
                    <br/><?= __('建议使用宽200像素*高60像素内的GIF或PNG透明图片；点击下方"确认提交"按钮后生效。') ?></p>
            </dd>
        </dl>
        <dl>
            <dt><?= __('APP Banner：') ?></dt>
            <dd>
                <p class="pic" style="width:180px;height:80px;"><img id="app_Banner_img"
                                                                     src="<?php if (!empty($re['app_banner'])) {
                                                                         echo $re['app_banner'];
                                                                     } ?>" height="80" width="180"/></p>
                <p class="upload-button"><input type="hidden" id="app_banner" name="shop[app_banner]"
                                                value="<?= $re['app_banner'] ?>"/>
                <div id='app_Banner_upload' class="lblock upload-bg"><i
                            class="iconfont icon-tupianshangchuan"></i><?= __('图片上传') ?></div>
                </p>
                <p class="hint"><?= __('此处为APP Banner；') ?>
                    <br/><?= __('建议使用宽200像素*高60像素内的GIF或PNG透明图片；点击下方"确认提交"按钮后生效。') ?></p>
            </dd>
        </dl>
            ...
            <dd>
                ...
                foreach ($method_list as $method) {
                    if (strpos($re['shop_shipping_method'], $method['shipping_method_id']) !== false) {
                        ...
                        <div style="width: 180px;float: left;">
                            <label style="margin-left: 20px;">
                                <input type="checkbox" checked name="shipping_method"
                    ...
                    <?php } else { ?>
                        <div style="width: 180px;float: left;">
                            <label style="margin-left: 20px;">
                                <input type="checkbox" name="shipping_method"
                                       value="<?php print($method['shipping_method_id']); ?>"
                                       id="check_<?php print($method['shipping_method_id']); ?>"><?php print($method['shipping_method_Name']); ?>
                            </label>
                        </div>
        ...
        <dl>
            <dt><?= __('服务时间：') ?> </dt>
            <dd>
                <input type="text" name="start_date" id="start_date" class="text w70"
                       value="<?= explode("-", $re['shop_service_time'])[0] ?>" placeholder="开始时间"/><em
                        class="add-on add-on2"><i class="iconfont icon-rili"></i></em>
                -
                <input type="text" name="end_date" id="end_date" class="text w70"
                       value="<?= explode("-", $re['shop_service_time'])[1] ?>" placeholder="结束时间"/><em
                        class="add-on add-on2"><i class="iconfont icon-rili"></i></em>
            </dd>
        ...
        <dl>
            <dt><?= __('店铺特色服务描述：') ?></dt>
            <dd><input type="text" class="text" name="shop[shop_description]" value="<?= $re['shop_description'] ?>"/>
            </dd>
        </dl>
        <dl>
            <dt><?= __('配送费设置：') ?></dt>
            <dd>满 <input type="text" name="shop[shop_shipping_conditions]" id="shop_shipping_conditions"
                         class="text w70" value="<?= $re['shop_shipping_conditions'] ?>"/> 元起送，
                配送费 <input type="text" name="shop[shop_deliver_fee]" id="shop_deliver_fee" class="text w70"
                           value="<?= $re['shop_deliver_fee'] ?>"/>
            </dd>
        </dl>
        <!--         <dl>
                    <dt>店铺头像：</dt>
        ...
        <dl>
            <dt><?= __('店铺条幅：') ?></dt>
            <dd>
                <p class="pic" style="max-width:800px;height:150px;"><img id="banner_img"
                                                                          src="<?php if (!empty($re['shop_banner'])) {
                                                                              echo $re['shop_banner'];
                                                                          } ?>" height="150"/></p>
                <p class="upload-button"><input type="hidden" id="banner" name="shop[shop_banner]"
                                                value="<?= $re['shop_banner'] ?>"/>
                <div id='banner_upload' class="lblock upload-bg"><i
                            class="iconfont icon-tupianshangchuan"></i><?= __('图片上传') ?></div>
                </p>
        ...
        <?php if ($shop_domain['shop_domain']['config_value']) {
            ...
            <dl>
                <dt><?= __('二级域名：') ?> </dt>
                <dd>
                    <input type="text" class="text" name="shop[shop_domain]" value="<?= $re['shop_domain'] ?>"
                           <?php if ($domain_list['shop_edit_domain'] <= 0){ ?>readonly="readonly"<?php } ?> />
                    <?php if ($shop_domain['is_modify']['config_value']) { ?><p
                            class="hint"><?php if ($domain_list['shop_edit_domain'] > 0) { ?><?= __('可留空，域名长度应为:') ?><?= $shop_domain['domain_length']['config_value'] ?>  <?= __('还可以修改') ?><?= $domain_list['shop_edit_domain'] ?><?= __('次') ?><?php } else { ?><?= __('修改次数已达上线') ?><?php } ?></p>
                    <?php } else { ?>
        ...
        <dl>
            <dt><?= __('店铺公告：') ?></dt>
            <dd>
                <textarea name="shop[shop_notice]" id="shop_notice" rows="2" class="textarea w200"
                          style="width:200px;"><?= $re['shop_notice'] ?></textarea>
            </dd>
        </dl>
        <dl>
            <dt></dt>
