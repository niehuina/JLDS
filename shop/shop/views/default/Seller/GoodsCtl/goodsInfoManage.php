	...
	<div class="form-style">
		<form method="post" id="form">
			...
			<dl>
				<dt><i>*</i><?=__('商品名称')?>：</dt>
				<dd>
                    <input type="hidden" name="erp_material_fnumber" value="<?php echo $erp_material_fnumber ?>"/>
					<input type="text" maxlength="32" name="name" class="text w450" value="<?php if( !empty($common_data) ) { echo $common_data['common_name']; } else if(!empty($erp_material_name) ){ echo $erp_material_name;}  ?>"
                    <?php if(!empty( $erp_material_fnumber)  ||  !empty( $common_data['erp_material_fnumber'] ) ) {echo "readonly"; } ?>
                    />
			...
			<dl>
				<dt><?=__('副标题')?>：</dt>
				<dd>
					<textarea class="text textarea w450" maxlength="32" name="promotion_tips"><?php if( !empty($common_data) ) { echo $common_data['common_promotion_tips']; } ?></textarea>
			...
			<dl>
				...
				<dd>
					<input type="text" class="text w60" id="price" name="price"
                           value="<?php if( !empty($common_data) ) { echo $common_data['common_price']; } ?>"
                            <?php if(!empty($common_data) && @$common_data['common_parent_id']>0 && @$common_data['product_is_allow_price'] !=1){?> readonly="readonly" <?php }?>/>
            ...
            <dl>
                <dt><i>*</i><?=__('会员价格')?>：</dt>
                <dd>
                    <input type="text" class="text w60" id="price_vip" name="price_vip" value="<?php if( !empty($common_data) ) { echo $common_data['common_price_vip']; } ?>" <?php if(!empty($common_data) && @$common_data['common_parent_id']>0 && @$common_data['product_is_allow_price'] !=1){?> readonly="readonly" <?php }?>/><em><?=Web_ConfigModel::value('monetary_unit')?></em>
                    <p class="hint"><?=__('价格必须是0.01~9999999之间的数字，且不能高于商品价格。')?>
                        <br/><?=__('此价格为商品的实际销售会员价格，如果商品存在规格，该价格显示最低会员价格。')?>
                        <?php if(!empty($common_data) && @$common_data['common_parent_id'] > 0){?>
                            <br/><?=__('修改价格范围为')?>：<?=$common_data['goods_recommended_min_price'] ?> - <?=$common_data['goods_recommended_max_price']?>
                        <?php }?>
                    </p>
            ...
            <dl>
                <dt><i>*</i><?=__('股东价格')?>：</dt>
                <dd>
                    <input type="text" class="text w60" id="price_partner" name="price_partner" value="<?php if( !empty($common_data) ) { echo $common_data['common_price_partner']; } ?>" <?php if(!empty($common_data) && @$common_data['common_parent_id']>0 && @$common_data['product_is_allow_price'] !=1){?> readonly="readonly" <?php }?>/><em><?=Web_ConfigModel::value('monetary_unit')?></em>

                    <p class="hint"><?=__('价格必须是0.01~9999999之间的数字，且不能高于会员价格。')?>
                        <br/><?=__('此价格为商品实际销售股东价格，如果商品存在规格，该价格显示最低股东价格。')?>
                        <?php if(!empty($common_data) && @$common_data['common_parent_id'] > 0){?>
                            <br/><?=__('修改价格范围为')?>：<?=$common_data['goods_recommended_min_price'] ?> - <?=$common_data['goods_recommended_max_price']?>
                        <?php }?>
                    </p>
...
<!--			</dl>-->
			...
			<dl class="special-01" nctype="virtual_valid" style="">
				...
				<dd>
					...
					<span></span>
					<p class="hint"><?=__('服务商品可兑换的有效期，过期后商品不能购买，电子兑换码不能使用。')?></p>
				</dd>
			</dl>
			<dl class="special-01" nctype="virtual_valid" style="">
				<dt><?=__('支持过期退款')?>：</dt>
...
<!--			</dl>-->
            <dl>
                <dt><?=__('是否热销')?>：</dt>
                <dd>
                    <label class="radio"><input  type="radio" name="is_best_seller" <?php if( !empty($common_data) && $common_data['common_is_best_seller'] == 0 ) { echo "checked"; } ?>  value="0"><?=__('否')?></label>
...
<!--            </dl>-->
            <dl>
                ...
                <dd>
                    ...
                    <select name="goods_returns_id" class="sgcategory valid">
                        <option value=""><?=__('请选择')?>...</option>
                        <?php if ( !empty($goods_return_list) ) { ?>
                            <?php foreach ($goods_return_list['items'] as $key => $val) { ?>
                                <option  value="<?= $val['returns_setting_id']; ?>"> <?= $val['returns_setting_Name']; ?></option>
                            <?php } ?>
                        <?php } ?>
                    </select>
...
<script>
	...
	$(function () {
		...
		/* 只有可发布服务商品才会显示 S */
		$('#form').validator("setField", "g_vindate", "<?=__('服务商品有效期')?>:required;");
...
//		$('th[nctype="spec_name_<?//= $spec_id ?>//"]').html("<?//= $spec_name ?>//");
        ...
        //初始化退换货设置
        <?php if ( !empty($common_data['goods_returns_setting']) ) {?>
        $('select[name="goods_returns_id"]').children('option[value="<?= $common_data['goods_returns_setting'] ?>"]').prop('selected', 'selected');
        <?php }  ?>
