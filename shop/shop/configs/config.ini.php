...
//订单时间配置
if (is_file(INI_PATH . '/time_api_' . $server_id . '.ini.php'))
{
    include_once INI_PATH . '/time_api_' . $server_id . '.ini.php';
}
else
{
    include_once INI_PATH . '/time_api.ini.php';
}
//设置未付款订单的取消时间(秒)
Yf_Registry::set('wait_pay_time', $wait_pay_time);

//设置系统自动确认收货的时间(秒)
Yf_Registry::set('confirm_order_time', $confirm_order_time);
    ...
    if(Web_ConfigModel::value('subsite_is_open') == 1){
        ...
        if(!isset($_COOKIE['area']) || !$_COOKIE['area']){
            ...
            if(isset($cur_area['province']) && $cur_area['province']){
                $province = str_replace('省', '', $cur_area['province']);
                setcookie("area",$province);
                $_COOKIE['area'] = $province;
            }
        }
    }
}
