...
$(document).ready(function(){
	...
	$(".receipt_address li").click(function(){
		...
		//如果是用户自提，不需要计算运费
       if($('input:radio:checked').parent('label').text().trim()!="自提"){
			getTransport();
       }else{
          var id= $(this).attr('id').substr(4);
           $("#shop option[value='" + id + "']").attr("selected", true);
       }
	});
    ...
    if(cartids){
        var cartidList = cartids.split(',');
        cartidList.forEach(function(v){
            var thisCheckBox = $(".checkitem[value='" + v + "']")[0];

            $(thisCheckBox).prop('checked', "true");
            $(thisCheckBox).parent().parent().parent().addClass('item-selected');

            //判断该店铺下的商品是否已全选
            if($(thisCheckBox).parents('.table_list').find(".checkitem").not("input:checked").length == 0)
            {
                $(thisCheckBox).parents(".carts_content").find(".checkshop").prop('checked', true);
            }
	...
	$("#pay_btn").click(function(){
		...
		var isShopAddr=false;
		if(method=="自提"){
            address_id   =$('#shop_address .add_choose').find("#address_id").val();
            address_contact = $('#shop_address .add_choose').find("h5").html();
            address_address = $('#shop_address .add_choose').find("p").html();
            address_phone   = $('#shop_address .add_choose').find(".phone").html();
            isShopAddr=true;
            if($('#shop').val()=="0"){
                Public.tips.error('请选择实体店铺！');
                return false;
			}

		}
		else {
			address_id   = $('#user_address .add_choose').find("#address_id").val();
			address_contact = $('#user_address .add_choose').find("h5").html();
			address_address = $('#user_address .add_choose').find("p").html();
			address_phone   = $('#user_address .add_choose').find(".phone").html();
            isShopAddr=false;
            if(!address_contact)
            {
                $('.add_address').click();
		...
		$(".select_increase").each(function(){
			if($(this).is(':checked'))
			{
				...
				var str = {
					...
					goods_id:$(this).parents('tr:eq(0)').find(".increase_num").attr('goods_id'),
		...
		$(".select").each(function(){
			if($(this).find('option').is(":selected"))
			{
				...
				{
					// shop_id = $(this).find("option:selected").attr('shop_id');
					// redId = $(this).find("option:selected").attr('red_id');
					// if (redId) {
