...
$(document).ready(function(){
	...
	//再来一单
	window.againOrder = function(o)
	{
        $.ajax({
            url: SITE_URL  + '?ctl=Buyer_Cart&met=againOrderToAddCart&typ=json',
            data:{order_id:o},
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            async:false,
            success:function(a){
                ...
                if(a.status == 200)
                {
                    window.location.href = SITE_URL + "?ctl=Buyer_Cart&met=cart&cartids=" + a.data.cart_ids;
                }
                else
                {
                    if(a.msg != 'failure')
