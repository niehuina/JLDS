...
class Sub_SiteModel extends Sub_Site
{
    ...
    public function getIPLoc_sina_new($queryIP){
        ...
        $url = 'http://api.map.baidu.com/location/ip?ip='.$queryIP.'&ak=A83cd06b54e826075981aa381d52b943';
        $ch = curl_init($url);
        ...
        curl_close($ch);
        return $location['content']['address_detail'];
    }
    ...
    public function getLbsGeo($address,$level=4){
        ...
        if(!$area){
            ...
            if(in_array($area_array['province'], array('北京','上海','天津','重庆'))){
                ...
                $area_array['city'] = trim($area_array['province'],'市');
