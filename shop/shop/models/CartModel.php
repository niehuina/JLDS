...
//            }
			...
			if ($user_id)
			{
				...
				$order_goods_cond['order_goods_status:in'] = $order_goods_status;
			...
			}else if($val['buy_limit'] && $IsHaveBuy){
                $val['buy_residue'] = 0;
            }
...
//                //获取实体店铺list
//                //获取good_id
                $shopEntityModel = new Shop_EntityModel();
                $shopInfo= $shopEntityModel->listByWhere(array("shop_id" => $val['shop_id']));
                $shopEntityList=$shopInfo['items'];

//                //
                $baseDistrictModel = new Base_DistrictModel();
                $entityList=null;
                foreach ($shopEntityList as $key=>$entity){
                    //$district_id = $entity['district_id'];
                    $city=$baseDistrictModel->getByWhere(array("district_name" => $entity['city']));
                    $shopEntityList[$key]['shop_address_province_id']=current($city)['district_parent_id'];
                    $shopEntityList[$key]['shop_address_city_id']=current($city)['district_id'];
                    $area=$baseDistrictModel->getByWhere(array("district_name" => $entity['district']));
                    $shopEntityList[$key]['shop_address_area_id']=current($area)['district_id'];

                }
                $shipping_methodModel = new Shipping_MethodModel();
                $method_ids=array_values(explode(",",$shop_base['shop_shipping_method']));
                $method_list = $shipping_methodModel->getByWhere(array('shipping_method_id:IN' => $method_ids));
                foreach($method_list as $key => $method){
                    $method_list[$key]['shop_id']=$val['shop_id'];
                }
                $data[$val['shop_id']]['shopEntityList'][] =$shopEntityList;
                //  /获取当前店铺的配送方式
                $data[$val['shop_id']]['method_list'][] =$method_list;
