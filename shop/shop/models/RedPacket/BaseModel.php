...
class RedPacket_BaseModel extends RedPacket_Base
{
    ...
    //获取用户可使用的店铺平台优惠券
    public function getUserRedPacketByWhere($user_id, $is_support_all, $order_price=null)
    {
        $cond_row                     	= array();
        $cond_row['redpacket_is_support_all'] = $is_support_all ? 1 : 0;
        $cond_row['redpacket_owner_id'] = $user_id;
        $cond_row['redpacket_state']    = self::UNUSED;
        if($order_price)
        {
            $cond_row['redpacket_t_orderlimit:<='] = $order_price;
        }
