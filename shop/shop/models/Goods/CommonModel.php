...
class Goods_CommonModel extends Goods_Common
{
		...
		else
		{
		    if($order_row['GPS']){
                $sql = "
					...
					FROM
						" . TABEL_PREFIX . "goods_common c LEFT OUTER JOIN " . TABEL_PREFIX . "shop_entity e ON c.shop_id = e.shop_id AND c.common_verify=1 AND c.common_state=1 AND c.shop_status=3 And e.lat >0 
					";
               ...
               $order = ' order by ' .
                   '(6378137.0*ACOS(SIN('. $order_row['GPS_LAT']  . '/180*PI())*SIN(e.Lat/180*PI())+COS(' . $order_row['GPS_LAT'] .'/180*PI())*COS(e.Lat/180*PI())*COS(('
                   .$order_row['GPS_LNG'] . '-e.Lng)/180*PI()))) ';
				...
				foreach ($goods_rows as $key => $goods_row)
				{
					...
					{                        
                        ...
                        if($user_grade == '2'){
                            ...
                            $goods_row['goods_price'] = $goods_row['goods_price_partner'];
                            $common_rows[$goods_row['common_id']]['common_price']  = $common_rows[$goods_row['common_id']]['common_price_partner'];
                        }
	...
	public function getGoodsByCommonId($cond_row = array(), $order_row = array(),$flag = true)
	{
		$type = 'SKU';
