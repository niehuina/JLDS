...
class Buyer_OrderCtl extends Buyer_Controller
{
    ...
    public function confirmOrder()
    {
        ...
        if ($typ == 'e') {
            ...
            if ($order_base['buyer_user_id'] == Perm::$userId && $order_base['order_status'] == Order_StateModel::ORDER_WAIT_CONFIRM_GOODS) {
                ...
                $edit_stock = $User_Stock_Model->editStockFromOrder($order_goods_data, $order_base['buyer_user_id'], $order_base['buyer_user_name']);
                check_rs($edit_stock, $rs_row);
    ...
    public function virtual()
    {
        ...
        if ($act == 'detail') {
            ...
            $order_row['buyer_user_id'] = $user_id;
            $order_row['order_buyer_hidden:<'] = Order_BaseModel::IS_BUYER_REMOVE;
            $order_row['order_is_virtual'] = Order_BaseModel::ORDER_IS_VIRTUAL; //服务订单
    ...
    public function addOrder()
    {
        ...
        if (Web_ConfigModel::value('Plugin_Directseller')) {
            ...
            //$user_parent = $User_InfoModel->getOne($user_id);

            //用户上级合伙人Id
            @$directseller_p_id = $User_InfoModel->getParentPartnerId($user_id);;  //二级

            //用户上级高级合伙人Id
            $user_g_parent_id = $User_InfoModel->getParentId($user_id);
            //$user_g_parent = $User_InfoModel->getOne($user_id);
            @$directseller_gp_id = $user_g_parent_id; //三级
        }
        ...
        foreach ($data as $ckey => $cval) {
            ...
            //每家店铺实际支付金额
            $shop_order_goods_row[$ckey]['shop_pay_amount'] = round(((($shop_order_goods_row[$ckey]['shop_sumprice'] - $shop_order_goods_row[$ckey]['shop_mansong_discount']) * $shop_order_goods_row[$ckey]['user_rate']) / 100) - $shop_order_goods_row[$ckey]['voucher_price'], 2);
            //每家店铺最后优惠金额
            $shop_order_goods_row[$ckey]['shop_user_rate'] = round(((($shop_order_goods_row[$ckey]['shop_sumprice'] - $shop_order_goods_row[$ckey]['shop_mansong_discount']) * (100 - $shop_order_goods_row[$ckey]['user_rate'])) / 100) - $shop_order_goods_row[$ckey]['voucher_price'], 2);
        }
...
//            }
        ...
        foreach ($shop_order_goods_row as $key => $val) {
            ...
            $order_row['order_is_virtual'] = 0;    //1-服务订单 0-实物订单
            ...
            foreach ($val['goods'] as $k => $v) {
                ...
                $order_goods_row['goods_returns_setting'] = $common_base['goods_returns_setting'];  //退换货设置
...
//			echo '<pre>';print_r([$shop_total_price*100, $increase_rule_info['rule_price']*100, $goods_info, $val['increase_goods_num'], $increase_arr]);exit;
            //判断当前购物车店铺商品是否满足加价购条件,
//			if((($shop_total_price*100 - $increase_rule_info['rule_price']*100) > 0) || (($shop_total_price*100 - $increase_rule_info['rule_price']*100) == 0))
//			{
            //一个店铺可以对应多个加价购商品，判断当前商品是否在返回的数组中
...
//		echo '<pre>';print_r($increase_shop_price);exit;
    ...
    public function addVirtualOrder()
    {
        ...
        $address_id = request_int('address_id', 0);
        $appointment_date = request_string('appointment_date');
...
//		echo '<pre>';print_r($shop_sumprice);exit;
        //店铺实际支付金额
...
//		echo '<pre>';print_r($shop_pay_amount);exit;
        fb($shop_sumprice);
        fb($shop_pay_amount);
        fb($shop_user_rate);
        fb('价格1');
        ...
        //
        $order_row['order_rpt_price'] = $address_id;    //平台红包抵扣订单金额
        $order_row['appointment_date'] = $appointment_date;    //平台红包抵扣订单金额
        $address_id = request_int('address_id', 0);
        ...
        $order_row['order_is_virtual'] = 1;    //1-服务订单 0-实物订单
        $order_row['order_shop_benefit'] = $order_shop_benefit;  //店铺优惠
        $order_row['payment_id'] = $pay_way_id;
        $order_row['payment_name'] = $PaymentChannlModel->payWay[$pay_way_id];
        ...
        $order_goods_row['order_goods_benefit'] = $order_goods_benefit;
        $order_goods_row['order_goods_time'] = get_date_time();
        $order_goods_row['goods_returns_setting'] = $data['common_base']['goods_returns_setting'];
...
//				//判断订单是否使用平台红包，如果使用，将平台红包状态改为未使用
    ...
    public function addChainOrder()
    {
        ...
        $order_goods_row['order_goods_benefit'] = $order_goods_benefit;
        $order_goods_row['order_goods_time'] = get_date_time();
        $order_goods_row['goods_returns_setting'] = $data['common_base']['goods_returns_setting']; //退换货设置
...
//        }
    ...
    public function oftenBuy()
    {
        ...
        $Order_GoodsModel = new Order_GoodsModel();
        $data = $Order_GoodsModel->getOftenBuyGoodsByUser($condi, $cat_id, $page, $rows);

        if ($data['items']) {
            foreach ($data['items'] as $key => $item) {
                $goods_id = $item['goods_id'];
                $Goods_Base = new Goods_BaseModel();
                $goods_base = $Goods_Base->getOne($goods_id);

                $cat_id = $goods_base['cat_id'];
                if ($cat_id) {
                    $data['items'][$key]['cat_id'] = $goods_base['cat_id'];
                    $data['items'][$key]['goods_base'] = $goods_base;

                    $Goods_Common = new Goods_CommonModel();
                    $common_base = $Goods_Common->getOne($goods_base['common_id']);
                    $data['items'][$key]['common_base'] = $common_base;
                }
            }
        }

        $goods_cats = $Order_GoodsModel->getOftenBuyGoodsCatByUser($user_id);
        $goods_cats = array_filter($goods_cats, function ($val) {
            return $val['cat_id'];
        });
...
//            $order_commission_0 = array_sum($directseller_commission_0_array);
            ...
            foreach($goods_list as $k=>$v) {
                if ($v['goods_return_status'] == 0 && ($v['goods_refund_status'] == 0 || $v['goods_refund_status'] == 3)
                    ...
                    && $v['directseller_flag']) {
                    $order_commission_0 += $v['directseller_commission_0'];  //一级分佣
...
//                $directseller_commission[2] += $v['directseller_commission_2'];  //三级分佣
                    ...
                    $order_commission_0 += $v['directseller_commission_0'] / $order_goods_num * $order_goods_real;  //一级分佣
                }
            }
            ...
            $order_list['items'][$key]['order_payment_amount'] = $order['order_payment_amount']*1 - $order['order_refund_amount'];
        }
    ...
    public function get_order_profit_partner()
    {
        $User_InfoModel = new User_InfoModel();
        $Order_BaseModel = new Order_BaseModel();
     ...
     * @throws Exception
     */
    public function get_order_profit_partner1()
    {
        $User_GradeModel = new User_GradeModel();
        $User_InfoModel = new User_InfoModel();
        $Order_BaseModel = new Order_BaseModel();
        ...
        if ($user_info['user_grade'] == 3) {
            //合伙人只计算其直属下级用户的所有订单
//            $user_children_ids = $User_InfoModel->getUserDirectChildrenNoPartner($user_id);
//            Yf_Log::log($user_children_ids, Yf_Log::LOG, 'user_settle_search');
...
//            $cond_row['buyer_user_id:in'] = explode(',', $user_children_ids);
            ...
            $user_orders = $Order_BaseModel->listByWhere($cond_row, $order_row, $page, $rows);
            Yf_Log::log("partner order:" . count($user_orders), Yf_Log::LOG, 'user_settle_search');

            if (count($user_orders['items']) > 0) {
                //计算其直属下级用户的未结算的订单总额
                ...
                Yf_Log::log("partner user_order_amount:" . $user_order_amount, Yf_Log::LOG, 'user_settle_search');
                if ($user_order_amount >= 0) {
                    //获取合伙人的返利比例
                    $grade_order_amount = $user_grade['order_amount'] * 1;
                    $grade_order_rebate1 = $user_grade['order_rebate1'] * 1 / 100;
                    $grade_order_rebate2 = $user_grade['order_rebate2'] * 1 / 100;
                    ...
                    foreach ($user_orders['items'] as $k => $order) {
                        ...
                        Yf_Log::log("partner total_amount:".$before_order_total_amount, Yf_Log::LOG, 'user_settle_search');
...
//                    if ($order_rebate_value_temp == 0) continue;
                        ...
                        $user_orders['items'][$k]['order_payment_amount'] = $order['order_payment_amount']*1 - $order['order_refund_amount'];
                    }
                }
            }
        } else if ($user_info['user_grade'] == 4) {
            //计算高级合伙人的提成比例
            $grade_order_rebate1 = $user_grade['order_rebate1'] * 1 / 100;
            $grade_order_rebate2 = $user_grade['order_rebate2'] * 1 / 100;
            $grade_order_rebate_top = $user_grade['order_rebate_top'] * 1 / 100;
