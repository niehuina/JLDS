...
class Goods_GoodsCtl extends Controller
{
	...
	public function goodslist()
	{
        ...
        $user_id = Perm::$userId;
        $address_contidion_row['user_id'] = $user_id;
        $userAddressModel     = new User_AddressModel();
        $userAddressList = $userAddressModel->getAddressList($address_contidion_row);
...
//暂时取消搜索时间限制
//		if(!$_COOKIE['guest_id'] || $_COOKIE['seash_keywords']){
//			$_REQUEST['keywords'] = "";
//		}
 
//		setcookie('seash_keywords',1,time()+3);
		...
		if ($search)
        {
            ...
            //记录用户搜素记录
            $user_recordModel = new User_RecordModel();
            $keyword_data                      = array();
            $keyword_data['user_id']=Perm::$userId;
            $keyword_data['user_record_keyword']=$search;
            $keyword_data['user_record_date']=date("Y-m-d H:i:s");
            $returnValue=$user_recordModel->addRecord($keyword_data);
        }
        if($sear_row){
        ...
        if($act == 'location'){
            $order_row['GPS'] = $actorder;
            $address_id =  request_int('ad_id');
            $addressinfo = $userAddressModel -> getAddressInfo($address_id);
            $order_row['GPS_LAT'] = $addressinfo['lat']; //'39.95354307391025';
            $order_row['GPS_LNG'] =  $addressinfo['lng']; // '119.59736700000788';
        }
	...
	public function test(){
        $k3cloud  = K3Cloud_Sync::instance();
        $location = $k3cloud -> test( );
        $this->data->addBody(-140, $location);
    }
	/**
	 * 获取商品详情页面商品详情
	 *
...
//				$goods_detail['recImages'] = isset($data['Plugin_Directseller_recGoods']) ? $data['Plugin_Directseller_recGoods'] : '';
		...
		if ('json' == $this->typ)
		{
			...
			$data['consumption_tax'] = Web_ConfigModel::value("consumption_tax");
            $data['value_added_tax'] = Web_ConfigModel::value("value_added_tax");
...
//				$data['common_ids'] = $common_ids;
    ...
    public function chain()
    {
		...
		if('json' == request_string("typ"))
		{
			...
			elseif($goods_num < 1 || $goods_num > $goods_shop_info['goods_stock'])
			{
				$msg = '购买数量有误！';
				$status = 250;
			}
