...
class Distribution_Seller_SettingCtl extends Seller_Controller
{
	...
	function directsellerDetail()
	{
		...
		$goods_data = $Goods_CommonModel->getGoodsList($cond_goods_row,array('common_id' => 'DESC'), $page, $rows);
		//将分销的商品id放在一个数组
		foreach($goods_data['items'] as $gkey=>$gval)
		{
			foreach($gval['good'] as $gk=>$gv)
			{
		...
		$order_goods_data = $Order_GoodsModel->getBaseList($cond_goods_ids_row, array(), $page, $rows);
		$order_ids = array_column($order_goods_data['items'], 'order_id');
