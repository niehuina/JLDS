...
class Seller_Shop_SetshopCtl extends Seller_Controller {
    ...
    public function shopCertification()
    {
        ...
        $shop_id['shop_id'] = Perm::$shopId;
        $re                = $this->shopBaseModel->getBaseOneList($shop_id);
        $re['certification']    = $re['shop_certification'] ? explode(',', $re['shop_certification']) : "";
        $arryCount=0;
        if(!empty($re['certification'])){
            foreach($re['certification'] as $key){
                if(!empty($key)){
                    $arryCount++;
                }
            }
        }
        $re['arryCount']=$arryCount;
        $re['status']="未认证";
        if($re['shop_certification_status']==1){
            $re['status']="待审核";
        }
        else if($re['shop_certification_status']==2){
            $re['status']="已认证";
        }
        else if($re['shop_certification_status']==3){
            $re['status']="未通过";
        }
        if ('json' == $this->typ)
        {
    ...
    public function editShopCertification()
    {
        $data['shop_certification']    = implode(",", request_row("certification"));
        $data['shop_certification_status']="1";//0：表示未认证 1：表示已经提交认证资料 2：审核通过 3：未通过
        $shop_id               = Perm::$shopId;
        $flag                  = $this->shopBaseModel->editBase($shop_id, $data);
        if ($flag === false)
        {
