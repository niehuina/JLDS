...
class Seller_GoodsCtl extends Seller_Controller
{
    ...
    public function add()
    {
        ...
        if ($cat_id) {
            if (empty($common_id)) {
                ...
                if (false) {
                    //自营店铺要先选择erp同步过来的物料（商品），不允许自己添加商品
                    if (request_string('erp_material_fnumber') != '') {
                        $erp_material_fnumber = request_string('erp_material_fnumber');
                        $erp_material_name = request_string('erp_material_name');

                        $goods_return_model = new Goods_ReturnsModel();
                        $goods_return_list = $goods_return_model->listByWhere(array(), array(), 1, 100);
                        $this->view->setMet('goodsInfoManage');
                    } else {
                        $this->view->setMet('materialSelection');
                    }
                } else {
                    $goods_return_model = new Goods_ReturnsModel();
                    $goods_return_list = $goods_return_model->listByWhere(array(), array(), 1, 100);
    ...
    {
        $erp_materialmodel = new ERP_MaterialModel();
        ...
        $data = $erp_materialmodel->getMaterialList($cond_row, array(), $page, $rows);
    ...
    public function addOrEditShopGoods()
    {
        ...
        if ($shop_base['shop_self_support'] != Shop_BaseModel::SELF_SUPPORT_TRUE) {
            $msg = $this->checkShopGoodsUpNum($shop_base['shop_id'], $action == "edit");
        ...
        $common_data['common_price_vip'] = request_float('price_vip');                   //商品会员价格
        $common_data['common_price_partner'] = request_float('price_partner');               //商品股东价格
        $common_data['common_market_price'] = request_float('market_price');                //市场价
        $common_data['common_dibao_price'] = request_float('dibao_price');                 //低保价
        $common_data['common_normal_price'] = request_float('normal_price');                //普通售价
        ...
        $common_data['common_is_best_seller'] = request_int('is_best_seller');
        $common_data['common_is_discount'] = request_int('is_discount');
        $common_data['common_is_preference'] = request_int('is_preference');
        if (request_string('erp_material_fnumber')) {
            $common_data['erp_material_fnumber'] = request_string('erp_material_fnumber');
        }

        $common_data['common_invoices'] = 1;                    //商品默认支持开发票
        if(Web_ConfigModel::value('Plugin_Directseller')) {//商品默认支持佣金计算
        ...
        $goods_returns_id = request_int('goods_returns_id');

        if (!empty($goods_returns_id)) {
            $common_data['goods_returns_setting'] = $goods_returns_id;                                //shop_goods_cat_id
        }
        ...
        if ($is_gv == 1) {
            $common_data['common_is_virtual'] = $is_gv;                                            //服务商品
            $common_data['common_virtual_date'] = request_string('g_vindate');                        //服务商品有效期
        ...
        if ($action == 'edit') {
            ...
            if ($shop_base['shop_type'] == 1 && $common_base['product_is_allow_update'] == 0 && $common_base['product_is_allow_price'] == 1) {
                ...
                $dist_allow_edit['common_dibao_price'] = $common_data['common_dibao_price'];
                $dist_allow_edit['common_normal_price'] = $common_data['common_normal_price'];
                ...
                if ($shop_base['shop_type'] == 1 && $common_base['product_is_allow_update'] == 1 && $common_base['product_is_allow_price'] == 0) {
                    ...
                    unset($common_data['common_dibao_price']);
                    unset($common_data['common_normal_price']);
        ...
        if ($common_id && $this->goodsCommonModel->sql->commitDb()) {
            ...
            if (Web_ConfigModel::value('im_statu')) {
                ...
                $formvars['goods_price_vip'] = $common_data['common_price_vip'];                  //商品会员价格
                $formvars['goods_price_partner'] = $common_data['common_price_partner'];              //商品股东价格
...
//			echo '<pre>';print_r($spec_data);exit;
            ...
            if (!empty($spec_data) && $flag_spec) {
                ...
                foreach ($spec_data as $key => $val) {
                    ...
                    $goods_data['goods_price_vip'] = $val['price_vip'];                  //商品会员价格
                    $goods_data['goods_price_partner'] = $val['price_partner'];              //商品股东价格
                    ...
                    $goods_data['goods_max_sale'] = request_int('limit');                   //单人最大购买数量
                ...
                $goods_data['goods_price_vip'] = $common_data['common_price_vip'];            //商品会员价格
                $goods_data['goods_price_partner'] = $common_data['common_price_partner'];        //商品股东价格
            ...
            if ($shop_base['shop_type'] == 2) {
                ...
                foreach ($all_common as $key => $value) {
                    ...
                    if ($common_base['product_is_allow_update']) {
                        ...
                        $allow_edit['common_price_vip'] = $dist_common_row['common_price_vip'];
                        $allow_edit['common_price_partner'] = $dist_common_row['common_price_partner'];
                        $allow_edit['common_market_price'] = $dist_common_row['common_market_price'];
                        $allow_edit['common_dibao_price'] = $dist_common_row['common_dibao_price'];
                        $allow_edit['common_normal_price'] = $dist_common_row['common_normal_price'];
            ...
            if ($data_goods['shop_id'] == $shop_id) {
                if (request_string('act') == 'down') {
                    ...
                    if (request_string('me') == 'lockup') {
                        ...
                        //不是自营店铺要检查店铺限制上架商品的数量
                        if ($shop_base['shop_self_support'] != Shop_BaseModel::SELF_SUPPORT_TRUE) {
                            $msg = $this->checkShopGoodsUpNum($shop_id, false);
...
//			$KTransportTypeName_VId = array_column($transport_type_rows, 'transport_area_id', 'transport_type_name');
            ...
            foreach ($excel_data as $key => $row_data) {
                ...
                $goods_is_virtual = $row_data[17];//服务商品
...
//				}
            ...
            if (!empty($import_goods_rows)) {
                foreach ($import_goods_rows as $import_goods_data) {
                    ...
                    if ($common_id) {
                        ...
                        //      $goods_data['goods_dibao_price'] 	= $import_goods_data['common_dibao_price'];        //市场价
    ...
    public function downloadTemplate()
    {
        ...
        for ($i = 4; $i < 99; $i++) {
            ...
            $objValidation_R = $active_sheet->getCell("R$i")->getDataValidation();    //服务商品
     ...
     * @return mixed|string
     */
    private function checkShopGoodsUpNum($shop_id, $isEdit = false)
    {
        $msg = "";
        $Goods_CommonModel = new Goods_CommonModel();
        $goods_state_normal_num = $Goods_CommonModel->getCommonStateNum($shop_id, Goods_CommonModel::GOODS_STATE_NORMAL);
        $goods_verify_wait_num = $Goods_CommonModel->getCommonVerifyNum($shop_id);
        $goods_up_num = $goods_state_normal_num + $goods_verify_wait_num;
        //如果商品进行编辑，则商品已上架数量减去1
        if ($isEdit) {
            $goods_up_num -= 1;
        }
        ...
        if (0 != $shop_grade_goods_limit && $shop_grade_goods_limit <= $goods_up_num) {
            $msg = __('商品发布数量限制！');
        }
        return $msg;
    }
