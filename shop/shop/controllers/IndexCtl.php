			...
			if ( !empty($layout_list) )
			{
				foreach($layout_list as $mb_tpl_layout_id => $layout_data_val)
				{
                    ...
                    if ($layout_data_val['mb_tpl_layout_type'] == 'home5')
                    {
                        ...
                        $hom5['title'] = $layout_data_val['mb_tpl_layout_title'];
                        $hom5['image'] = $mb_tpl_layout_data['image'];
                        $hom5['type']  = $mb_tpl_layout_data['image_type'];
                        $hom5['data']  = $mb_tpl_layout_data['image_data'];

                        $data[$mb_tpl_layout_id+1]['home5'] = $hom5;
                    }
...
//			{
                ...
                $floor_images = $this->get_floor_image();
...
//		echo '<pre>';print_r($data);exit;
	...
	public function toolbar()
	{
        ...
        if (Perm::checkUserPerm())
        {
            ...
            $voucher_base_row['voucher_owner_id']          = Perm::$userId;
            $voucher_base_row['voucher_state']             = Voucher_BaseModel::UNUSED;
            $voucher_base_row['voucher_end_date:>=']       = get_date_time();
            $voucher_base_order_row['voucher_state']       = 'ASC';
            $vouchers_get = $voucherBaseModel->getUserVoucherList($voucher_base_row, $voucher_base_order_row);
            ...
            $shop_vouchers_row = $voucherTempModel->getVoucherTempList($voucher_row, $voucher_order_row);
        }
...
//					echo '<pre>';print_r($init_rs['data']);exit;
	...
	private function get_floor_image()
    {
        $floor_image = array();

        //母婴用品
        $image['url']="/index.php?ctl=Goods_Goods&met=goodslist&cat_id=20";
        $image['src']="f_my.jpg";
        $floor_image[] = $image;

        //服饰箱包
        $image['url']="/index.php?ctl=Goods_Goods&met=goodslist&cat_id=42";
        $image['src']="f_xb.jpg";
        $floor_image[] = $image;

        //个护美妆
        $image['url']="/index.php?ctl=Goods_Goods&met=goodslist&cat_id=7";
        $image['src']="f_gh.jpg";
        $floor_image[] = $image;

        //生鲜食品
        $image['url']="/index.php?ctl=Goods_Goods&met=goodslist&cat_id=31";
        $image['src']="f_sx.jpg";
        $floor_image[] = $image;

        //运动户外
        $image['url']="/index.php?ctl=Goods_Goods&met=goodslist&cat_id=3";
        $image['src']="f_sh.jpg";
        $floor_image[] = $image;
