...
class Api_Trade_OrderCtl extends Api_Controller
{
     ...
     *使用平台红包下单成功，更改状态
     */
    public function updateStateForRP()
    {
        $redpacket_id = request_string('redpacket_id');
        $order_id   = request_string('order_id');

        if($redpacket_id)
        {
            $RedPacket_BaseModel        = new RedPacket_BaseModel();
            $flag = $RedPacket_BaseModel->changeRedPacketState($redpacket_id, $order_id);
     ...
     * 使用优惠券下单成功，更改状态
     */
    public function updateStateForVoucher()
    {
        $voucher_id = request_string('voucher_id');
        $order_id   = request_string('order_id');

        if($voucher_id)
        {
            $Voucher_BaseModel = new Voucher_BaseModel();
            $flag = $Voucher_BaseModel->changeVoucherState($voucher_id, $order_id);
