...
class Api_Seller_Trade_OrderCtl extends Api_Controller
{
	...
	public function orderList()
	{
		...
		$user_info = $User_InfoModel->getOneByWhere(['user_id'=>$user_id]);
		//获取订单状态表中的状态id数组
		...
		else
		{
			...
			{
				$data = $Order_BaseModel->getBaseList(['buyer_user_id'=>$buyer_user_id], $order_row, $page, $rows);
			}
			else
			{
		...
		foreach($data['items'] as $key=>$val)
		{
			...
			$data['items'][$key]['user_logo'] = $user_info['user_logo'];
		}
	...
	public function searchOrder()
	{
		...
		if($search_type == 'buyer')//如果是按照用户昵称搜索，则返回用户id
		{
			...
			$user_data = $User_BaseModel->getOneByWhere(['user_account'=>$search_word]);
			$data['id'] = $user_data['user_id'];
			$data['type'] = 'buyer';
		}
		else
		{
...
//			{
//				$data['order_shop_benefit'] = explode(' ', $data['order_shop_benefit']);
//			}
...
//            $check_send = $this->checkSend($order_base['seller_user_id'],$order_base['shop_id']);
	...
	{
		$field_row                                 = array();
		$field_row['shop_id']                      = request_int('shop_id');
		...
		{
//			$data['default_address'] = $Shop_ShippingAddressModel->getOneByWhere(['shop_id'=>$data['shop_id'], 'shipping_address_default'=>1]);
			//如果是设置为默认地址，则找出之前默认地址修改其默认状态
			if($field_row['shipping_address_default'] == 1)
			{
				$default_addr = $Shop_ShippingAddressModel->getOneByWhere(['shop_id'=>$field_row['shop_id'], 'shipping_address_default'=>1]);
				if($default_addr)
				{
					$Shop_ShippingAddressModel->updateAddress($default_addr['shipping_address_id'], ['shipping_address_default'=>0]);
				}
			}
			...
			if ($flag !== false)
			{
				...
				$edit_rows['order_seller_address'] = $field_row['shipping_address_address'].' '.$field_row['shipping_address_area'];
		...
		if (request_string('op') == 'edit')
		{
			...
			$flag = $Shop_ShippingAddressModel->updateAddress($ship_id, $field_row);
			if ($flag !== false)
			{
				$msg    = __('success');
				$status = 200;
