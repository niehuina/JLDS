...
//
    ...
    public function getShopGoodsByUserId()
    {
        ...
        if($shop_info)
        {
            ...
            $data = $this->goodsCommonModel->getByWhere($cond_rows);
            if($data)
            {
                //$data = array_values($data);
                foreach($data as $key=>$value)
                {
                    $goods = $this->goodsBaseModel->getOneByWhere(['common_id'=>$value['common_id']]);
                    $data[$key] = $goods;
                    ...
                    $data[$key]['common_dibao_price'] = $goods_common['common_dibao_price'];
    ...
    public function getUserShopCompany()
    {
        ...
        $this->data->addBody(-140, $shop_company);
    }
    ...
    public function reduceOrAddStock()
    {
        ...
        {
            $type = '增加';
            $flag = $Goods_BaseModel->editBase($goods_id, ['goods_stock'=>$goods_num]);
            $flag1 = $Goods_CommonModel->editCommon($common_info['common_id'], ['common_stock'=>$goods_num],true);
        }
        else
        {
            $type = '减少';
            $flag = $Goods_BaseModel->editBase($goods_id, ['goods_stock'=>(-1)*$goods_num]);
            $flag1 = $Goods_CommonModel->editCommon($common_info['common_id'], ['common_stock'=>(-1)*$goods_num],true);
        }
    ...
    public function editShopCompany()
    {
        ...
        if(request_string('business_id'))
        {
            ...
            {
                return $this->data->setError("请填写营业执照所在地");
            }*/
