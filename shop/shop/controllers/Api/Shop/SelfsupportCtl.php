...
class Api_Shop_SelfsupportCtl extends Api_Controller
{
     ...
     * @return mixed
     */
    private function getOrganizationList($shop_id, $order_row=array())
    {
        $shop_row = array();
        if($shop_id){
            $shop_row['shop_id:not in'] = array($shop_id);
        }
        $shopBaseInfo = $this->shopBaseModel->getByWhere($shop_row, $order_row);
        $usedOrganization = array_filter(array_column($shopBaseInfo, 'shop_organization'));
        $cond_row = array();
        $cond_row['code:not in'] = $usedOrganization;
        $erp_organization = new ERP_OrganizationModel();
        $organizationlist  = $erp_organization->getOrganizationList($cond_row,array(),1,200);
        return $organizationlist['items'];
    }
}
