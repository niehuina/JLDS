...
class Api_Shop_ManageCtl extends Api_Controller
{
	...
	public function getinformationrow()
	{
        ...
        $method_list = $shipping_methodModel->getByWhere();

		$data['class'] = $this->shopClassModel->getClassWhere();
		$data['grade'] = $this->shopGradeModel->getGradeWhere();
        $erp_organization = new ERP_OrganizationModel();
        $organizationlist  = $erp_organization->getOrganizationList(array(),array(),1,200);
        $data['organizaion_list'] = $organizationlist['items'];
...
//        $edit_shop_row['shop_organization'] = request_int("shop_organization");
        $edit_shop_row['shop_shipping_method'] = request_string("shop_shipping_method");
		$shop_id       = request_int('shop_id');
    ...
    public function shopCertification()
    {
        ...
        $cond_row["shop_certification_status:in"]=  array("1","3");
        if ($shop_account)
        {
            if ($shop_type)
            {
...
//        $data['base'][$shop_id]['organization_code'] = "'".$data['base'][$shop_id]['organization_code']."'";
    ...
    {
        $shop_id  = request_int('shop_id');
        $edit_status['shop_certification_status'] = request_int('shop_verify');
        $edit_status['shop_certification_reason'] = request_string('shop_certification_reason');
        $flag = $this->shopBaseModel->editBase($shop_id, $edit_status);
