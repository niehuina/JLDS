...
class Api_Article_BaseCtl extends Api_Controller
{
	...
	public function addArticleBase()
	{
        ...
        $data['article_app_flag'] = request_string('article_app_flag');

        if($data['article_app_flag']=="1")//设为App 文章,则将之前的APP文章修改为普通文章即可
        {
            $cond_row  = array();
            $cond_row['article_app_flag'] ="1";
            $existActList  = $Article_BaseModel->getByWhere($cond_row);

            if(!empty($existActList)){
                foreach ($existActList as $key =>$item)
                {
                    $item_value['article_app_flag']="0";
                    $returnFlag=$Article_BaseModel->editBase($item['article_id'], $item_value);
                }
            }
