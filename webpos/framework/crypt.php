...
/**
 * CodeIgniter Encryption Class，本类无需单独调用
 *  
...
class crypt {
	...
	public function __construct(array $params = array())
	{
		...
		if ( ! isset($this->_key) && self::strlen($key = config::get('app.crypt')?:'encode') > 0)
		{
			$this->_key = $key;
		}
	}
	...
	protected function _mcrypt_initialize($params)
	{
		...
		if ( ! empty($params['mode']))
		{
			...
			{
				throw new Exception( 'Encryption: MCrypt mode '.strtoupper($params['mode']).' is not available.');
