...
{  
	...
	* 生成HTML静态文件的文件夹名
	*/
	static $htmlcache = "upload/cache_html";
	/**
	* theme最上层目录名
	*/
	...
	*/
	public function __construct(){    
 	...
 	public function action($id,$arg = [] ){
 			$u  = url_array();
 			return url($u['module'].'/'.$u['controller'].'/'.$id,$arg);
 	}
	/**
	* 加载view 同级内容
	* @param string $name  
	...
	* @return void 
	*/
	public function extend($name,$par = []){ 
	...
	public function load_file($file,$par = []){  
	...
	*/
	protected function find($arr = []){
		...
		throw new Exception(__('view file not exists') );
	...
	*	渲染视图同make
	*/
	public function render($name, $par = [])
	{  

		hook::listen('view.before_render',$par);
		...
		$data = trim(ob_get_contents());   
		ob_end_clean();
		hook::listen('view.after_render',$data);
		if(static::$cache!== false && static::$cache >= 0){
			$url = static::cacheHtml();   
			file_put_contents($url,$data); 
		} 
	...
	* @return void 
	*/
	public function layout($name , $par = [] ){  
	...
	*@return void
	*/
	public function start($name , $par = []){
		$this->block_id = $name;  
		ob_start();
		ob_implicit_flush(false);  
		extract($par, EXTR_OVERWRITE); 
	}
	public function block($name,$par = []){
		return $this->start($name,$par);
	}
	/**
	* 关闭 加载指定的 content
	*/
