...
class router{
	...
	static function pathinfo(){
		...
		if($mod == 1 || $mod == 2){
			if($mod == 1){
				...
				$pa = array_filter(explode(config('app.url_pathinfo')?:'/',$pathinfo));
				$_GET['m'] = $pa[0];
				$_GET['c'] = $pa[1];
				$_GET['a'] = $pa[2];

				unset($pa[0],$pa[1],$pa[2]);
				if($pa){ 
					$get = array_values($pa);
					...
					foreach ($get as $key => $value) {
						...
						if($i%2 == 0 && !in_array($value, ['a','c','m'])){
							$_GET[$value] = $get[$key+1];	
						} 
					   $i++; 
					}
				}
			...
			$a = $_GET['a'];
			if($m&&$c&&$a){ 
				 //open pathinfo url link 
				  $class = "modules\\".$m."\\".$c;
				  static::$pathinfo =   [$class,$a]; 
			} 
	...
	*/
	protected function create_url($url,$par = []){
		...
		if($mod == 1){ 
			  ...
			  $ur = $url.substr($po,0,-1);
			  $ur = str_replace('/',config('app.url_pathinfo')?:'/', $ur);
 			  return base_url().'/index.php/'.$ur;
		}elseif($mod == 2){
			  $arr = explode('/',$url);
			  $key = ['m','c','a'];
			  $builder = array_combine($key,$arr)+$par;
 			  return base_url().'/index.php?'.http_build_query($builder);
		}
	...
	*/
	protected function exec(){  
		self::pathinfo();
		if(static::$pathinfo ){
			$cs = static::$pathinfo[0];
			$fun = static::$pathinfo[1];
			return (new $cs)->$fun();
	...
	*/
	protected function load_route($class,$ac,$data){  
		...
		$run =  call_user_func_array([$obj,$ac],$data); 
