		...
		<div class="reg-form fl">
			<form action="" id="register-form" method="post" novalidate="novalidate" onsubmit="return false;">
				...
				<div class="form-item" id="form-item-authcode">
					<label>验　证　码：</label>
					<input type="text" autocomplete="off" name="authcode" id="form-authcode" maxlength="4" class="field form-authcode" placeholder="请输入正确的验证码" default="<i class=&quot;i-def&quot;></i>输入正确的后才可以发送短信" onfocus="showTip(this)" onblur="checkCode()">
...
//        }
        ...
        if(reg_checkcode == 1 || reg_checkcode == 3){
            if(mobile == ""){
                Public.tips.alert('手机号不能为空');
                $("#re_user_mobile").focus();
                return;
            }
            var user_code = $("#phoneCode").val();
            if(user_code == ""){
                Public.tips.alert('手机验证码不能为空');
                $("#phoneCode").focus();
                return;
            }
        }else{
            if(email == ""){
                Public.tips.alert('邮箱不能为空');
                $("#re_user_email").focus();
                return;
            }
            var user_code = $("#emailCode").val();
            if(user_code == ""){
                Public.tips.alert('邮箱验证码不能为空');
                $("#emailCode").focus();
                return;
            }
        }
        if(user_password == ""){
            Public.tips.alert('密码不能为空');
