            ...
            <form action="" id="register-form" method="post" novalidate="novalidate" onsubmit="return false;"
                ...
                <div class="pas">
                    <div id="form-item-password" class="form-item" style="z-index: 12;">
                        ...
                        <input type="password" id="re_user_password" class="field re_user_password" maxlength="20"
                               placeholder="建议至少使用两种字符组合密码" default="<i class=i-def></i><?= $pwd_str ?>"
                               onfocus="checkPwd()" onblur="pwdCallback()">
                ...
                <div id="form-item-rpassword" class="form-item disb">
                    ...
                    <input type="password" name="form-equalTopwd" id="form-equalTopwd" class="field"
                           ...
                           onblur="checkRpwd()" onfocus="showTip(this)">
                    <span class="clear-btn eye-icon"></span>
                    <i class="i-status"></i>
                    <span class="close-btn js_clear_btn clear-icon"></span>
                    ...
                    <div id="Mobile" style="display: <?= $mobile_display ?>">
                        <div class="item-phone-wrap">
                            <div class="form-item form-item-mobile" id="form-item-mobile">
                                ...
                                <input type="text" id="re_user_mobile" class="field re_user_mobile"
                                       placeholder="建议使用常用手机" maxlength="11"
                                       default="<i class=&quot;i-def&quot;></i>完成验证后，可以使用该手机登录和找回密码"
                                       onblur="checkMobile()" onfocus="showTip(this)">
                                <span class="clear-btn js_clear_btn clear-icon"></span>
                                <i class="i-status"></i>
                        ...
                        <div class="form-item" id="form-item-authcode">
                            ...
                            <input type="text" name="authcode" id="form-authcode" maxlength="6"
                                   class="field form-authcode" placeholder="请输入验证码"
                                   default="<i class=&quot;i-def&quot;></i>看不清？点击图片更换验证码" onfocus="showTip(this)"
                                   onblur="checkCode()">
                            <img onClick="get_randfunc(this);" title="换一换" class="img-code" style="cursor:pointer;"
                                 src='./libraries/rand_func.php'/>
                        </div>
                        <div class="input-tip disb">
                            <span></span>
                        </div>
                        <div class="form-item form-item-phonecode">
                            ...
                            <input type="text" name="mobileCode" maxlength="6" id="phoneCode"
                                   class="field phonecode  re_mobile" placeholder="请输入手机验证码">
                            <button id="getPhoneCode" class="btn-phonecode" type="button" onclick="get_randcode()">
                                获取验证码
                            </button>
                            <i class="i-status"></i>
                        </div>
                ...
                <div id="Email" style="display: <?= $email_display ?>">
                    <div class="item-phone-wrap">
                        <div class="form-item form-item-email" id="form-item-email">
                            ...
                            <input type="text" id="re_user_email" class="field re_user_email" placeholder="建议使用常用邮箱"
                                   default="<i class=&quot;i-def&quot;></i>完成验证后，可以使用该邮箱登录和找回密码" onblur="checkEmail()"
                    ...
                    <div class="form-item form-item-phonecode">
                        ...
                        <input type="text" name="emailCode" maxlength="6" id="emailCode"
                               class="field emailcode  re_email" placeholder="请输入邮箱验证码">
                        <button id="getEmailCode" class="btn-phonecode" type="button" onclick="get_randcode()">获取验证码
                        </button>
                        <i class="i-status"></i>
                    </div>
                </div>
                ...
                <?php foreach ($reg_opt_rows as $opt_row): ?>
                    <div class="form-item  clearfix form-item-<?= $opt_row['reg_option_id'] ?>"
                         id="form-item-<?= $opt_row['reg_option_id'] ?>">
                        <label><?php if ($opt_row['reg_option_required'] == 1) { ?><em
                                    class="must">*</em><?php } ?><?= $opt_row['reg_option_name'] ?></label>

                        <?php if ($opt_row['option_id'] == 1): ?>
                            <select id="re_user_<?= $opt_row['reg_option_id'] ?>"
                                    name="option[<?php echo $opt_row['reg_option_id']; ?>]"
                                    class="field select-drap field-reset re_user_<?= $opt_row['reg_option_id'] ?>"
                                    maxlength="20" placeholder="<?= $opt_row['reg_option_placeholder'] ?>"
                                    default="<?= $opt_row['reg_option_placeholder'] ?>"
                                    data-datatype="<?= $opt_row['reg_option_datatype'] ?>">
                                <?php
                        ...
                        <?php elseif ($opt_row['option_id'] == 2): ?>
                            ...
                            <div class="clearfix cont-area">
                                <?php foreach ($reg_option_value_row as $k => $option_value) { ?>
                                    <label class="pad0">
                                        <input type="radio" name="option[<?php echo $opt_row['reg_option_id']; ?>]"
                                               value="<?php echo $k; ?>"/>
                        ...
                        <?php elseif ($opt_row['option_id'] == 3): ?>
                            ...
                            <div class="clearfix cont-area">
                                <?php foreach ($reg_option_value_row as $k => $option_value) { ?>
                                    <label>
                                        <input type="checkbox" name="option[<?php echo $opt_row['reg_option_id']; ?>]"
                                               value="<?php echo $k; ?>"/>
                                        <?php echo $option_value; ?>
                                    </label>
                                <?php } ?>
                        ...
                        <?php elseif ($opt_row['option_id'] == 4): ?>
                            ...
                            <input type="text" id="re_user_<?= $opt_row['reg_option_id'] ?>"
                                   name="option[<?php echo $opt_row['reg_option_id']; ?>]"
                                   class="field field-reset re_user_<?= $opt_row['reg_option_id'] ?>" maxlength="20"
                                   placeholder="请输入<?= $opt_row['reg_option_name'] ?>"
                                   default="<?= $opt_row['reg_option_placeholder'] ?>"
                                   data-datatype="<?= $opt_row['reg_option_datatype'] ?>" onblur="checkFormatter(this)"
                                   onfocus="showTip(this)">

                        <?php elseif ($opt_row['option_id'] == 5): ?>
                            <textarea type="text" id="re_user_<?= $opt_row['reg_option_id'] ?>"
                                      name="option[<?php echo $opt_row['reg_option_id']; ?>]"
                                      class="field field-reset re_user_<?= $opt_row['reg_option_id'] ?>"
                                      placeholder="请输入<?= $opt_row['reg_option_name'] ?>"
                                      default="<?= $opt_row['reg_option_placeholder'] ?>"
                                      data-datatype="<?= $opt_row['reg_option_datatype'] ?>"
                                      onblur="checkFormatter(this)" onfocus="showTip(this)"></textarea>
...
//                }
        ...
        register_obj = $.extend(register_obj, {
