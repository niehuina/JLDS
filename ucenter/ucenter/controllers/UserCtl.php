...
class UserCtl extends Yf_AppController
{
	...
	public function getUserInfo()
	{
		...
		$data = $this->userInfoModel->getInfoDetail($user_id);
		$data = $data[$user_id];
...
//		}
		...
		if (!$this->userInfoModel->getOne($user_id))
		{
...
//			$this->userInfoModel->sync($user_id);
	...
	public function friend()
	{
		...
		else
		{
			...
			if ($user_name)
			{
				...
				$user_list = $this->userBaseModel->getInfoList($cond_row, $order_row, $page, $rows);
				...
				if (!empty($data['items']))
				{
					foreach ($data['items'] as $key => $val)
					{
						...
						foreach ($tag['items'] as $k => $v)
						{
							...
							$detail                     = $this->userInfoModel->getOne($v['user_id']);
	...
	public function addFriends()
	{
		...
		{
			$user_id = $val['user_id'];
			$detail  = $this->userInfoModel->getInfoDetail($user_id);
...
//		$cond_row['user_name:!='] = $user_name;
	...
	public function editMobileInfo()
	{
        ...
        //检查用户信息
        $user_info_row = $this->userInfoModel->getOne($user_id);
        if (!$user_info_row)
        {
            $data['code'] = 3;
        ...
        $flag_edit = $this->userInfoModel->editInfoDetail($user_id, $edit_user_row);
	...
	public function editEmailInfo()
	{
        ...
        if($check_user_name){
            $data['code'] = 2;
            return $this->data->addBody(-140, $data, __('该邮箱已经被使用'), 250);
        }
	...
	public function unbind()
	{
		...
		$user_info_datail = $User_InfoDetailModel->getInfoDetail($user_id);
