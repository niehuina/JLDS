...
class LoginCtl extends Yf_AppController
{
	...
	public function index()
	{
		...
		if (Perm::checkUserPerm())
		{
			...
			$user_info = $User_InfoDetailModel->getInfoDetail($user_id);
			$user_info = current($user_info);
...
//				$User_InfoModel->check_input($user_name, $password, $user_mobile);
	...
	public function checkUserName()
	{
		...
		$user_id_row = $User_InfoDetail->checkUserName($user_name);
	...
	public function checkMobile()
	{
		...
		$data   = array();
		$result = array_column($checkmobile, 'user_name');
        $data = $checkmobile;
		if($checkmobile)
		{
            if(in_array($mobile,array_values($result))){
                $msg    = 'success';
                $status = 200;
            }
            else{
