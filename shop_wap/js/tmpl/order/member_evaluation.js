    ...
    $.getJSON(ApiUrl + "/index.php?ctl=Buyer_Order&met=evaluation&act=add&typ=json", {k: e,u:u, order_id: a}, function (r) {
        ...
        $(".btn-l").click(function () {
            ...
            for(var ts = 0; ts < order_goods_id_list.length; ts++) {
                ...
                evaluation_little.push(l["goods[" + order_goods_id + "][score]"]);   //score
                var score=l["goods[" + order_goods_id + "][score]"];
                if(score == 1){
                    evaluation_little.push('bad');
                }
                else if(score ==2 || score==3){
                    evaluation_little.push('neutral');
                }
                else{
                    evaluation_little.push('good');
                }
            ...
            $.ajax({
                ...
                data: {evaluation: evaluation, k:getCookie('key'), u: getCookie('id'), app: 1},
                dataType: "json",
                async: false,
                success: function (e) {
                    checkLogin(e.login);
                    ...
                    window.location.href = WapSiteUrl + "/tmpl/order/order_list.html"
                }
            })
        })
    })
