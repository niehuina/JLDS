            ...
            if(password!=spassword){
                ...
                $.post(url, {k: k, u: getCookie('id'),'yzm':yzm,'mobile':mobile}, function(a){
                    if (a.status == 200){
                        $.ajax({
                            type: "post",
                            url: ApiUrl+"/index.php?ctl=Buyer_User&met=editUserPayPassword&typ=json",
                            data: {k: k, u: getCookie('id'), user_id: user_id, password: password},
                            dataType: "json",
                            success: function (a) {
                                if (a.status == 200) {
                                    errorTipsShow("<p>设置支付密码成功</p>");
                                    location.href = WapSiteUrl + "/tmpl/member/member_setting.html"
                                }
                                else {
                                    errorTipsShow("<p>修改支付密码失败</p>");
                                }
                            }
                        })
                    }
