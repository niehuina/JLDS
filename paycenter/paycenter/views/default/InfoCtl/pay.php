	...
	<form style="padding-left:6px;">
		<div class="recharge2-content-top content-public clearfix">
			...
			<div class="box clearfix">
				...
				<div>
					<input type="checkbox" class="pay_yue" name="choice"  value="cards" id='cards_checkbox'><?=_('使用购物卡支付（可用余额：')?><?=format_money($user_resource['user_recharge_card'])?>）
				...
				<div>
					<input type="checkbox" class="pay_yue" name="choice"  value="money" id='money_checkbox'><?=_('使用预存款支付（可用余额：')?><?=format_money($user_resource['user_money'])?>）
				...
				<div>
					<input type="checkbox" class="pay_yue" name="choice" value="bt"><?=_('使用白条支付（可用额度：')?><?=format_money($bt_money)?>）
					<input type="hidden" name="bt" id="bt" value="<?=$bt_money?>">
					<!--  用白条支付的金额  -->
					<input type="hidden" name="bt_pay" id="bt_pay" value="0"/>
                    <span id="pay_yue_bt_span" style="margin-left:73px;color:red;">(注：白条不能与其他支付方式同时使用)</span>
				</div>
				<?php }?>
