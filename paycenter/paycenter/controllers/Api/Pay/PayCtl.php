...
//                'record_paytime' => date('Y-m-d H:i:s'),
    ...
    public function money()
    {
        ...
        $user_id = request_int('user_id');
        $amount = request_float('pay_amount');
        $order_id=request_string('order_id');

        $User_ResourceModel = new User_ResourceModel();
        $flag = $User_ResourceModel->frozenUserMoney($user_id,$amount);

        $UseInfoModel = new User_InfoModel();
        $user=$UseInfoModel->getInfo($user_id);
        $para                  = array();
        $para['user_id']=$user_id;
        $para['user_nickname']=current($user)['user_nickname'];
        $para['record_money']=$amount;
        $para['record_date']=date("Y-m-d H:i:s");

        $para['record_year']=date('Y');
        $para['record_month']=date('m');
        $para['record_day']=date('d');
        $para['record_title']='门店购物';
        $para['record_desc']='';
        $para['record_time']=date("Y-m-d H:i:s");
        $para['trade_type_id']=Trade_TypeModel::SHOPPING;//充值为3，提现为4，购物为1，付款为7
        $para['user_type']=1;
        ...
        $Consume_RecordModel = new Consume_RecordModel();
        $flag = $Consume_RecordModel->addRecord($para,true);
