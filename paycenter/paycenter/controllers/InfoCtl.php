...
class InfoCtl extends Controller
{
    ...
    public function recordlist()
    {
        ...
        if ($start_date) {
            $cond_row['record_date:>='] = $start_date;
        }
        if ($end_date) {
            $cond_row['record_date:<='] = $end_date;
        }
        if ($status) {
...
//		$Consume_RecordModel = new Consume_RecordModel();
    ...
    public function withdraw()
    {
        ...
        $configList = $Web_ConfigModel->getByWhere(array('config_type' => 'set', 'config_key' => 'cashamount_min'));
        $cashamount_min = 0;
        if (!empty($configList)) {
            $cashamount_min = current($configList)['config_value'];
        }
    ...
    public function pay()
    {
        ...
        //用于判断订单类型，order_g_type = physical实物订单，virtual服务订单
...
//        $banner = empty($banners['data']['banner_list']) ? null : $banners['data']['banner_list'][0];
        ...
        foreach ($payment_channel as $key => $val) {
            ...
            if ($val['payment_channel_code'] == 'cards' || $val['payment_channel_code'] == 'money' || $val['payment_channel_code'] == 'app_wx_native') {
                unset($payment_channel[$key]);
            }
        }
        if (isset($bt_info)) {
    ...
    public function checkPayWay()
    {
        ...
        if ($online_payway) {
            ...
            $Payment_ChannelModel = new Payment_ChannelModel();
            $payway = $Payment_ChannelModel->getOneByWhere(array('payment_channel_code' => $online_payway));
            $payment_channel_id = $payway ? $payway['payment_channel_id'] : $payment_channel_id;
        }
        ...
        if ($bt_payway == 'true') {
            ...
            $order_id_row = trim($uorder_base['inorder'], ',');
            $Consume_TradeModel = new Consume_TradeModel();
            $flag1 = $Consume_TradeModel->editConsumeTrade($order_id_row, array('payment_channel_id' => $payment_channel_id));
            check_rs($flag1, $rs_row);
        }
        ...
        $edit_union_order_row['payment_channel_id'] = $payment_channel_id;
    ...
    public function certification()
    {
        ...
        $UserBaseModel = new User_BaseModel();
        $User_InfoModel = new User_InfoModel();
        $base=$UserBaseModel->getOne($user_id);
        $data = $User_InfoModel->getOne($user_id);
        $data['user_name']=$base['user_account'];
        $from = request_string('from');
        include $this->view->getView();
    }
    ...
    public function addWithdraw()
    {
        ...
        $edit['amount'] = request_float('withdraw_money');//提现金额
        $edit['fee'] = request_float('fee');//提现手续费
...
//		else
    ...
    public function refundTransfer()
    {
        ...
        $return_order_code = request_string('return_order_code');
        ...
        if ($amount < 0) {
            ...
            if ($flag1) {
                if($uorder_base['payment_channel_id']==4){
                    $Payment_WxNativeModel = PaymentModel::create('wx_native');
                    $refund_data = array();
                    $refund_data['return_order_code'] = $return_order_code;
                    $refund_data['return_case'] = $amount * 100;
                    $refund_data['out_trade_no'] = $uorder_base['union_order_id'];
                    $refund_data['total_fee'] = $uorder_base['union_online_pay_amount'] * 100;
                    if($Payment_WxNativeModel->refund($refund_data))
                    {
                ...
                }else if($uorder_base['payment_channel_id']==Payment_ChannelModel::ALIPAY){
                    $Payment_AlipayModel = PaymentModel::create('alipay');
                    $refund_data = array();
                    $refund_data['return_case'] = $amount;
                    $refund_data['trade_no'] = $uorder_base['alipay_trade_no'];
                    if($Payment_AlipayModel->refund($refund_data)){
                        $user_resource_row['user_recharge_card'] = $user_resource['user_recharge_card'] + $card_return_amount;
                        $User_ResourceModel->editResource($user_id, $user_resource_row);
                        $flag = true;
                    }else{
    ...
    public function getWith()
    {
        ...
        $Consume_WithdrawModel = new Consume_WithdrawModel();
        $datas = $Consume_WithdrawModel->getByWhere();
    ...
    //低保认证
    public function minLivingCertification()
    {
        //获取用户信息
        $user_id = Perm::$userId;
        //$user_id = '1';
    ...
    public function editminLivingCertification()
    {
        //获取用户信息 '0未认证1待审核2已认证3审核失败'
        $user_id = Perm::$userId;
        $User_InfoModel = new User_InfoModel();
        $data = $User_InfoModel->getOne($user_id);
        if ($data['user_identity_statu'] != "2") {//实名认证没有审核或者未通过
            $msg = '请先通过实名认证！';
            $status = 250;
        } else {

            $edit['user_minimum_living_img'] = request_string('user_minimum_living_img');

            $edit['user_minimum_living_status'] = 1; //'0未认证1待审核2已认证3审核失败'
            $User_InfoModel = new User_InfoModel();
            $flag = $User_InfoModel->editInfo($user_id, $edit);
            if ($flag !== false) {
                $msg = 'success';
    ...
    public function getWithDrawInfoForWap()
    {
        $data = array();
        ...
        $userResourceModel = new User_ResourceModel;
        $user_resource_data = $userResourceModel->getOne(Perm::$userId);
        $data['user_resource'] = $user_resource_data;
        ...
        if (!empty($configList)) {
            $cashamount_min = current($configList)['config_value'];
            $data['cashamount_min'] = $cashamount_min;
        }
