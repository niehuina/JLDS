...
if ($verify_result)
{
	...
	if (isset($_GET['trade_status']))
	{
		if($_GET['trade_status'] == 'TRADE_FINISHED' || $_GET['trade_status'] == 'TRADE_SUCCESS')
		{
			...
			if($notify_row['deposit_total_fee'] != $data['trade_payment_amount'])
			{
				Yf_Log::log('金额错误'.$notify_row['order_id'], Yf_Log::ERROR, 'alipay_return_error_fee');
				die;
			}

			$trade_type = Trade_TypeModel::$trade_type_row[$data['trade_type_id']];
			Yf_Log::log("trade_type=" . json_encode($trade_type), Yf_Log::INFO, 'pay_alipay_return');
			...
			//web_pos
			if($trade_type == "web_pos")
			{
				Yf_Log::log("order_id=" . json_encode($notify_row['order_id']), Yf_Log::INFO, 'pay_alipay_return');
				$unionOrderModel = new Union_OrderModel();
				$flag = $unionOrderModel->editUnionOrder($notify_row['order_id'], ['order_state_id'=> Union_OrderModel::PAYED]);
			}
