...
if ($verify_result)
{
	...
	if ($notify_row)
	{
		...
		{
			Yf_Log::log('金额错误'.$notify_row['order_id'], Yf_Log::ERROR, 'alipay_error_fee');
			die;
		}
		...
		if($data['app_id'] == 207){
            ...
            if($result){
                ...
                Yf_Log::log('Process-SUCCESS', Yf_Log::INFO, 'pay_alipay_notify');
            }else{
                echo "FAIL";        //请不要修改或删除
                Yf_Log::log('Process-FAIL', Yf_Log::ERROR, 'pay_alipay_notify_error');
                Yf_Log::log('Process-FAIL', Yf_Log::ERROR, 'pay_alipay_notify');
            }
