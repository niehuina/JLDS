...
<script type="text/javascript" src="<?= $this->view->js ?>/libs/jquery/plugins/validator/jquery.validator.js"
        charset="utf-8"></script>
<script type="text/javascript" src="<?= $this->view->js ?>/libs/jquery/plugins/validator/local/zh_CN.js"
        charset="utf-8"></script>
</head>
...
<form method="post" enctype="multipart/form-data" id="shop_edit_class" name="form1">
    <div class="ncap-form-default">
        <dl class="row">
            ...
            <?php if ($_GET['type'] == 'bt') { ?>
                <dd class="opt">
                    <input type="radio" name="user_bt_status" value="1"
                           <?php if ($data['user_bt_status'] == 1){ ?>checked="checked"<?php } ?>>待审核
                    <input type="radio" name="user_bt_status" value="2"
                           <?php if ($data['user_bt_status'] == 2){ ?>checked="checked"<?php } ?>>通过
                    <input type="radio" name="user_bt_status" value="3"
                           <?php if ($data['user_bt_status'] == 3){ ?>checked="checked"<?php } ?>>不通过
                </dd>
            <?php } else { ?>
                <dd class="opt">
                    <input type="radio" name="user_identity_statu" value="1"
                           <?php if ($data['user_identity_statu'] == 1){ ?>checked="checked"<?php } ?>>待审核
                    <input type="radio" name="user_identity_statu" value="2"
                           <?php if ($data['user_identity_statu'] == 2){ ?>checked="checked"<?php } ?>>通过
                    <input type="radio" name="user_identity_statu" value="3"
                           <?php if ($data['user_identity_statu'] == 3){ ?>checked="checked"<?php } ?>>不通过
                </dd>
            <?php } ?>
        </dl>
    ...
    function initPopBtns() {
        ...
        api.button({
            id: "confirm", name: t[0], focus: !0, callback: function () {
                postData(oper, rowData.user_realname);
                return cancleGridEdit(), $("#shop_edit_class").trigger("validate"), !1
            }
        }, {id: "cancel", name: t[1]})
    }

    function postData(t, c) {
        $_form.validator({
