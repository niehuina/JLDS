...
<form method="post" enctype="multipart/form-data" id="shop_edit_class" name="form1">
    <div class="ncap-form-default">
        <dl class="row">
            ...
            <dd class="opt">
                <input type="radio" name="user_minimum_living_status" value="1" <?php if($data['user_minimum_living_status'] == 1){?>checked="checked"<?php }?>>待审核
                <input type="radio" name="user_minimum_living_status" value="2" <?php if($data['user_minimum_living_status'] == 2){?>checked="checked"<?php }?>>通过
                <input type="radio" name="user_minimum_living_status" value="3" <?php if($data['user_minimum_living_status'] == 3){?>checked="checked"<?php }?>>不通过
            </dd>
        </dl>
    ...
    function postData(t, e)
    {
        $_form.validator({
            ...
            valid: function (form)
            {
                ...
                var status= $.trim($("input[name='user_minimum_living_status']:checked").val());
                params ={
                    user_id:user_id,
                    status: status,
                };
                Public.ajaxPost(SITE_URL +"?ctl=Paycen_PayInfo&met=editMinInfoRow&typ=json", params, function (e)
                {
                    if (200 == e.status)
                    {
