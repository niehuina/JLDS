...
//		}
     ...
     * 订单时限设置
     */
	public function editTimeApi()
    {
        $time_api_row = request_row('time_api');

        $data                           = array();
        $data['wait_pay_time']          = $time_api_row['wait_pay_time'];
        $data['confirm_order_time']     = $time_api_row['confirm_order_time'];

        if (is_file(INI_PATH . '/time_api_' . Yf_Registry::get('server_id') . '.ini.php'))
        {
            $file = INI_PATH . '/time_api_' . Yf_Registry::get('server_id') . '.ini.php';
        }
        else
        {
            $file = INI_PATH . '/time_api.ini.php';
        }
        ...
        $formvars['app_id']        = $shop_app_id;
        $formvars['admin_account'] = Perm::$row['user_account'];
        $formvars['time_api'] = $data;
        $res = get_url_with_encrypt($key, sprintf('%s?ctl=Api_Config&met=editTimeApi&typ=json', $url), $formvars);
