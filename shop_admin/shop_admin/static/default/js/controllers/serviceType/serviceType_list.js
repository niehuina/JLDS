...
$(function() {
    ...
    var handle = {
        ...
        operate: function(oper, row_id) {
            ...
            $.dialog({
                title: title,
                content: 'url:' + SITE_URL + '?ctl=Base_ServiceType&met=manage&typ=e',
                data: data,
                width: 400,
                height: 165,
                max: false,
        ...
        del: function(row_ids) {
            $.dialog.confirm(_('删除的将不能恢复，请确认是否删除？'), function() {
                Public.ajaxPost(SITE_URL + '?ctl=Base_ServiceType&met=remove&typ=json', {
                    services_type_id: row_ids
                }, function(data) {
                    if (data && data.status == 200) {
                        var id_arr = data.data.services_type_id || [];
        ...
        callback: function(data, oper, dialogWin) {
            ...
            gridData[data.services_type_id] = data;

            if (oper == "edit") {
                $("#grid").jqGrid('setRowData', data.services_type_id, data);
                dialogWin && dialogWin.api.close();
            } else {
                $("#grid").jqGrid('addRowData', data.services_type_id, data, 'first');
    ...
    function initGrid() {
        ...
        $('#grid').jqGrid({
            url: SITE_URL + '?ctl=Base_ServiceType&met=getServiceTypeList&typ=json',
            datatype: 'json',
            autowidth: true,
            shrinkToFit: false,
            forceFit: true,
            ...
            sortname: "services_type_id", //指定默认排序的列
            ...
            loadComplete: function(response) {
                if (response && response.status == 200) {
                    ...
                    for (var i = 0; i < data.items.length; i++) {
                        ...
                        item['id'] = item.services_type_id;
                        gridData[item.services_type_id] = item;
                    }
