...
$(function() {
    ...
    var handle = {
        ...
        operate: function(oper, row_id) {
            ...
            $.dialog({
                title: title,
                content: 'url:' + SITE_URL + '?ctl=Base_GoodsReturns&met=manage&typ=e',
                data: data,
                width: 400,
                height: 165,
                max: false,
        ...
        del: function(row_ids) {
            $.dialog.confirm(_('删除的将不能恢复，请确认是否删除？'), function() {
                Public.ajaxPost(SITE_URL + '?ctl=Base_GoodsReturns&met=remove&typ=json', {
                    returns_setting_id: row_ids
                }, function(data) {
                    if (data && data.status == 200) {
                        var id_arr = data.data.returns_setting_id || [];
        ...
        callback: function(data, oper, dialogWin) {
            ...
            gridData[data.returns_setting_id] = data;

            if (oper == "edit") {
                $("#grid").jqGrid('setRowData', data.returns_setting_id, data);
                dialogWin && dialogWin.api.close();
                $("#grid").trigger("reloadGrid");
            } else {
                $("#grid").jqGrid('addRowData', data.returns_setting_id, data, 'first');
        ...
        operFormatter: function(val, opt, row) {
            var html_con="";
            var name = row.returns_setting_Name;
            if (name=='不可退' || name=='随时退' || name=='未服务随时退')
            {
                 html_con = '<div class="operating" data-id="' + row.id + '"></div>';
            }
            else {
                 html_con = '<div class="operating" data-id="' + row.id + '">' +
                    '<span class="ui-icon ui-icon-pencil" title="修改"></span>' +
    ...
    function initGrid() {
        ...
        var colModel = [{
            ...
            "name": "returns_setting_Name",
            "index": "returns_setting_Name",
            "label": "服务类型名称",
            "classes": "ui-ellipsis",
            "align": "center",
            "title": false,
        ...
        $('#grid').jqGrid({
            url: SITE_URL + '?ctl=Base_GoodsReturns&met=getReturnsList&typ=json',
            datatype: 'json',
            autowidth: true,
            shrinkToFit: false,
            forceFit: true,
            ...
            sortname: "returns_setting_id", //指定默认排序的列
            ...
            loadComplete: function(response) {
                if (response && response.status == 200) {
                    ...
                    for (var i = 0; i < data.items.length; i++) {
                        ...
                        item['id'] = item.returns_setting_id;
                        gridData[item.returns_setting_id] = item;
                    }
