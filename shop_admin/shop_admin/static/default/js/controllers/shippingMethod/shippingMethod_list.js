$(function() {
    ...
    var handle = {
        ...
        operate: function(oper, row_id) {
            ...
            $.dialog({
                title: title,
                content: 'url:' + SITE_URL + '?ctl=Base_ShippingMethod&met=manage&typ=e',
                data: data,
                width: 400,
                height: 165,
                max: false,
        ...
        del: function(row_ids) {
            $.dialog.confirm(_('删除的将不能恢复，请确认是否删除？'), function() {
                Public.ajaxPost(SITE_URL + '?ctl=Base_ShippingMethod&met=remove&typ=json', {
                    shipping_method_id: row_ids
                }, function(data) {
                    if (data && data.status == 200) {
                        var id_arr = data.data.shipping_method_id || [];
        ...
        callback: function(data, oper, dialogWin) {
            ...
            gridData[data.shipping_method_id] = data;

            if (oper == "edit") {
                $("#grid").jqGrid('setRowData', data.shipping_method_id, data);
                dialogWin && dialogWin.api.close();
            } else {
                $("#grid").jqGrid('addRowData', data.shipping_method_id, data, 'first');
        ...
        operFormatter: function(val, opt, row) {
            var name = row.shipping_method_Name;
            if (name=='自提' || name=='义工配送' || name=='专业配送')
            {
    ...
    function initGrid() {
        ...
        $('#grid').jqGrid({
            url: SITE_URL + '?ctl=Base_ShippingMethod&met=getMethodList&typ=json',
            datatype: 'json',
            autowidth: true,
            shrinkToFit: false,
            forceFit: true,
            ...
            sortname: "shipping_method_id", //指定默认排序的列
            ...
            loadComplete: function(response) {
                if (response && response.status == 200) {
                    ...
                    for (var i = 0; i < data.items.length; i++) {
                        ...
                        item['id'] = item.shipping_method_id;
                        gridData[item.shipping_method_id] = item;
                    }
