        ...
        $('#points-setting-form').validator({
            ...
            fields: {
               ...
               'points[points_recharge]': 'integer[+0];',
                'points[points_period]': 'integer[+0];',
...
$(function ()
{
    if ($('#set-setting-form').length > 0)
    {
        $('#set-setting-form').on("click", "a.submit-btn", function (e)
        {
            parent.$.dialog.confirm('修改立马生效,是否继续？', function ()
                {
                    Public.ajaxPost(SITE_URL + '?ctl=Config&met=edit&typ=json', $('#set-setting-form').serialize(), function (data)
                    {
                        if (data.status == 200)
                        {
                            parent.Public.tips({content: '修改操作成功！'});
...
$(function ()
{
    if ($('#mb-setting-form').length > 0)
    {
        $('#mb-setting-form').on("click", "a.submit-btn", function (e)
        {
            parent.$.dialog.confirm('修改立马生效,是否继续？', function ()
                {
                    Public.ajaxPost(SITE_URL + '?ctl=Config&met=edit&typ=json', $('#mb-setting-form').serialize(), function (data)
                    {
                        if (data.status == 200)
                        {
                            parent.Public.tips({content: '修改操作成功！'});
...
$(function()
{
    ...
    if ($('#time_api-setting-form').length)
    {

        $('#time_api-setting-form').validator({
            ignore: ':hidden',
            theme: 'yellow_bottom',
            timely: 1,
            stopOnError: true,
            fields: {
                'time_api[wait_pay_time]': 'required;',
                'time_api[confirm_order_time]': 'required;',
            },
            valid: function (form)
            {
                parent.$.dialog.confirm('修改立马生效,是否继续？', function ()
                    {
                        Public.ajaxPost(SITE_URL + '?ctl=Config&met=editTimeApi&typ=json', $('#time_api-setting-form').serialize(), function (data)
                        {
                            if (data.status == 200)
                            {
                                parent.Public.tips({content: '修改操作成功！'});
        ...
        }).on("click", "a.time-submit-btn", function (e)
        {
            $(e.delegateTarget).trigger("validate");
        });
