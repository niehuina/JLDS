...
<!--        </dl>-->
          ...
          <dt class="tit">
           ...
           <dd class="opt">
            ...
            <dl class="row">
                ...
                <dd class="opt">
                    <?php
                    $selectMethod=explode(",",$data['shop_shipping_method']);
                    foreach($data['method_list'] as $method){
                        if(strpos($data['shop_shipping_method'],$method['shipping_method_id']) !== false){

                            ?>
                            <div style="width: 180px;float: left;">
                                <label style="margin-left: 20px;">
                                    <input type="checkbox" checked  name="shipping_method" value="<?php print($method['shipping_method_id']);?>"
                        ...
                        <?php }else{ ?>
                            <div style="width: 180px;float: left;">
                                <label style="margin-left: 20px;">
                                    <input type="checkbox"  name="shipping_method" value="<?php print($method['shipping_method_id']);?>"
                                           id="check_<?php print($method['shipping_method_id']); ?>"><?php print($method['shipping_method_Name']);?>
                                </label>
                            </div>
                        <?php }} ?>
                    <input type="hidden" id="shop_shipping_method" name="shop_shipping_method" value="" />
                </dd>
            </dl>
        </div>
    </form>
    ...
    <script>
         ...
         //数据库取到的值设置为选中
<!--         --><?php //if ( !empty($data['shop_organization']) ) {?>
//         $('select[name="shop_organization"]').children('option[value="<?//= $data['shop_organization']?>//"]').prop('selected', 'selected');
//         <?php //}  ?>
    </script>
    <script>
	...
	$_form.validator({
        ...
        valid: function (form)
        {
            ...
            var 
                ...
                shop_shipping_method=$.trim($("#shop_shipping_method").val()),
			    n = "add" == t ? "新增店铺" : "修改店铺";
			...
			params = {
                ...
                //shop_organization:shop_organization,
